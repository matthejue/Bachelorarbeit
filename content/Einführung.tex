...
Auf diese Weise sind alle \colorbold{Statements} und \colorbold{Ausdrücke} in \colorbold{A-Normalform} (Definition~\ref{def:atomarer_ausdruck}).
...
\begin{Definition}{Atomarer Ausdruck}{atomarer_ausdruck}
  Ein \colorbold{Atomarer Ausdruck} ist ein Ausdruck, der \colorbold{atomar} ist, also \colorbold{nicht} mehr weiter \colorbold{zerkleinert} werden kann. Also entweder eine \colorbold{Variable} \smalltt{var}, eine \colorbold{Zahl} \smalltt{12}, ein \colorbold{ASCII-Zeichen} \smalltt{'c'} oder ein \colorbold{Zugriff auf eine Location}, wie z.B. \smalltt{stack(1)} ist.\footcite{g_siek_course_2022}
\end{Definition}
...
\begin{Definition}{A-Normalform (ANF)}{a_normal_form}
  ...
  Eine \colorbold{Konkrette Syntax} ist in \colorbold{A-Normalform}, wenn alle \colorbold{Komplexen Ausdrücke} nur \colorbold{Atomare Ausdrücke} enthalten und einer \colorbold{Location} zugewiesen sein müssen.
...
Ein solcher \colorbold{Pass} hat vor allem die Aufgabe sich der \colorbold{Syntax} von \colorbold{Maschinenbefehlen} besonders dadurch anzunähren, dass


dass mit \colorbold{Ausnahme} einiger \colorbold{spezieller Statements}, die in späteren Passes umgwandelt werden müssen, so weit wie möglich immer nur \colorbold{eine} Operation pro \colorbold{Statement} ausgeführt wird. Dadurch lassen sich viele Statements bereits direkt zu \colorbold{Maschinenbefehlen} übersetzen, da Machienenbefehle üblicherweise immer nur \colorbold{eine} Operation ausführen können\footnote{Z.B. kann eine \colorbold{RETI-CPU} in einem \colorbold{Fetch- und Execute-Zyklus} nur eine Operation mit der \colorbold{Arithmetisch Logischen Einheit} (engl. ALU) ausführen.}.

Alle weiteren denkbaren \colorbold{Passes} sind zu \colorbold{spezifisch} für bestimmte \colorbold{Statements} und \colorbold{Ausdrücke}, als das sich zu diesen allgemein etwas mit einer \colorbold{Theorie} dahinter sagen lässt. Alle \colorbold{Passes}, die zur Implementierung des \colorbold{PicoC-Compilers} geplant und ausgedacht wurden sind im Unterkapitel~\label{sec:passes} definiert.
