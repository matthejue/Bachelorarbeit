...
\begin{itemize}[itemsep=-1mm, topsep=-1mm]
  \coloritem[Top-Down Parsing] Es wird versucht durch Expansion der Produktionen der \colorbold{Konkretten Syntax} bis zu den Blättern, \colorbold{Linksableitungen} zu finden, sodass die \colorbold{Terminalsymbole} der \colorbold{Blätter} mit dem \colorbold{Inputstring} matchen\footnote{\colorbold{Rechtsableitungen} wären mit \colorbold{Recursive-Descent} auch möglich, wenn man dann auch über den Inputstring \colorbold{rückwärts} iteriert. Es gibt allerdings keinen guten Grund, warum man immer das \colorbold{rechteste} \colorbold{Non-Terminal} zuerst expandieren sollte, da für \colorbold{Kontextfreie Grammatiken} sowohl mit \colorbold{Links- als auch Rechtsableitungen} alle Wörter der Sprache, die die \colorbold{Kontextfreie Grammatik} beschreibt, ableitbar sind.}. Hierunter fallen:
  \begin{itemize}[itemsep=-1mm, topsep=-1mm]
    \coloritem[Recursive-Descent Parser]: Die Struktur dieser \colorbold{Parser} ist sehr nah an der \colorbold{Konkretten Syntax}, dessen Sprache sie erkennen sollen. Die richtigen \colorbold{Linksableitungen} für die \colorbold{Non-Terminale} der \colorbold{Konkretten Syntax}, welche oftmals nach den Regeln der Konkretten Syntax \colorbold{Nicht-deterministisch} mehrere Möglichkeiten der Expansion haben, werden entweder durch \colorbold{Backtracking} oder \colorbold{Vorausschauen}, um eine konstante Zahl $k$ von Tokens\footnote{Das wird auch als \colorbold{Lookahead} von $k$ bezeichnet.} gefunden.
    \begin{itemize}[label=\textbullet, itemsep=-1mm, topsep=-1mm]
      \coloritem Parser, die die \colorbold{erstere} Strategie implementieren, werden auch als \colorbold{Backtracking Parser bezeichnet}\footnote{Diesesr Art von Parser wurde für den \colorbold{PicoC-Compiler} implementiert, als dieser noch auf dem Stand des \colorbold{Bachelorprojektes} war, bevor er durch den nicht selbst implementierten \colorbold{Earlay Parser} von Lark ersetzt wurde.}. Backtracking Parser... TODO.
      \coloritem Parser, die die \colorbold{letztere} Strategie implementieren, werden auch als \colorbold{LL(k) Recursive-Descent Parser} bezeichnet\footnote{Diese Art von Parser ist im \colorbold{RETI-Interpreter} implementiert, da die \colorbold{RETI-Sprache} eine besonders simple \colorbold{LL(1) Grammatik} besitzt.}. Diese Parser erkennen \colorbold{LL(k) Grammatiken}, wobei eine Grammatik \colorbold{LL(k)} ist, wenn ein \colorbold{LL(k) Recognizer} dafür konstruiert werden kann, der diese erkennt. Dabei steht \colorbold{LL} für \colorbold{l}eft-to-right und \colorbold{l}eftmost-derivation, da die \colorbold{Token} von \colorbold{links nach rechts} gematched werden und immer das \colorbold{linkeste Non-Terminal} expandiert wird.
    \end{itemize}
  \end{itemize}
  \coloritem[Bottom-Up Parsing] Es wird mit dem Inputstring gestartet und versucht, die \colorbold{Folge von Ableitungen}, die zu seiner Entstehung führt durch Rückwärtsanwenden der \colorbold{Produktionen} der \colorbold{Konkretten Syntax} bis zum  \colorbold{Startsymbol} zurückzuverfolgen. Hierunter fallen:
  \begin{itemize}[itemsep=-1mm, topsep=-1mm]
    \coloritem[Earlay Parser]:
    \coloritem[LR Parsing]:
  \end{itemize}
\end{itemize}
\end{Special_Paragraph}
