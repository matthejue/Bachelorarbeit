...
\begin{Definition}{Pass}{pass}
  ...
  \begin{equation}
    \begin{tikzpicture}[auto, baseline=(current  bounding  box.center)]
      ...
      \draw[->] (program1) to[bend right] node[left, align=center] {interpretiere $A_{i}$\\ als Abstract Syntax Tree\\ der Sprache $L_{i}$} (output);
      \draw[->] (program2) to[bend left] node[right, align=center] {interpretiere $A_{i+1}$\\ als Abstract Syntax Tree\\ der Sprache $L_{i+1}$} (output);
    \end{tikzpicture}
    \label{eq:compiler_beziehungen}
  \end{equation}
wobei man hier so tut, als gäbe es diese zwei Interpreter, welche die beiden Abstract Syntax Trees interpretieren können.
