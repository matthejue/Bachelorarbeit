...
\begin{itemize}[itemsep=-1mm, topsep=-1mm]
  \coloritem[Top-Down Parsing] Es wird versucht durch Expansion der Produktionen der \colorbold{Konkretten Syntax} bis zu den Blättern, \colorbold{Linksableitungen} zu finden, sodass die \colorbold{Terminalsymbole} der \colorbold{Blätter} mit dem \colorbold{Inputstring} matchen\footnote{\colorbold{Rechtsableitungen} wären auch möglich, aber es gibt keinen guten Grund, warum man immer das rechteste \colorbold{Non-Terminal} zuerst expandieren sollte, da für \colorbold{Kontextfreie Grammatiken} sowohl mit Links- auch Rechtsableitungen alle Wörter der Sprache, die die Kotextfreie Grammatik beschreibt ableitbar sind.}. Hierunter fallen:
  \begin{itemize}[itemsep=-1mm, topsep=-1mm]
    ...
    \begin{itemize}[label=\textbullet, itemsep=-1mm, topsep=-1mm]
      \coloritem Parser, die die erstere Strategie implementieren, werden auch als \colorbold{Backtracking Parser bezeichnet}\footnote{Diesesr Art von Parser wurde für den \colorbold{PicoC-Compiler} implementiert, als dieser noch auf dem Stand des \colorbold{Bachelorprojektes} war, bevor er durch den nicht selbst implementierten \colorbold{Earlay Parser} von Lark ersetzt wurde.}. Backtracking Parser... TODO.
      \coloritem Parser, die die letztere Strategie implementieren, werden auch als \colorbold{LL(k) Recursive-Descent Parser} bezeichnet\footnote{Diese Art von Parser ist im \colorbold{RETI-Interpreter} implementiert, da dieser eine besonders simple $LL(1)$ Grammatik besitzt}. Diese Parser erkennen \colorbold{LL(k) Grammatiken}, wobei eine Grammatik \colorbold{LL(k)} ist, wenn ein \colorbold{LL(k) Parser} dafür konstruiert werden kann. Wobei \colorbold{LL} für \colorbold{left-to-right} und \colorbold{leftmost-derivation} steht, .
    \end{itemize}
  \end{itemize}
