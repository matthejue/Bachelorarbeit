...
In Abbildung~\ref{fig:lexikalische_analyse_veranschaulichung} ist die gesamte \colorbold{Lexikalische Analyse} nochmal abschließend veranschaulicht, um eine Gesamtübersicht zu geben.

\begin{figure}[h]
  \codebox[title=Inputstring, remember as=inputstring, width=0.2\linewidth]{./code_examples/example1.picoc}
  \hfill
  \treebox[title=Tokenfolge, remember as=tokenfolge, width=0.6\linewidth]{./code_examples/example1.tokens}
...
\begin{figure}[h]
  \treebox[title=Tokenfolge, remember as=Tokenfolge, width=0.8\linewidth]{./code_examples/example1.picoc}
  \vspace{3cm}
  \treebox[title=Derivation Tree, remember as=derivation_tree, width=0.8\linewidth]{./code_examples/example1.dt}
  \vspace{3cm}
  \treebox[title=Abstract Syntax Tree, remember as=abstract_syntax_tree, width=0.8\linewidth]{./code_examples/example1.ast}

  \begin{tikzpicture}[overlay,remember picture,line width=1mm,draw=PrimaryColor]
    \draw[->] (tokenfolge.south) to[bend down] node[above] {Parser} (derivation_tree.north);
    \draw[->] (derivation_tree.south) to[bend down] node[above] {Visitor und Transformer} (abstract_syntax_tree.north);
  \end{tikzpicture}
