...
Im Falle dessen, dass es sich bei der \colorbold{Sprache} $L_1$ um eine \colorbold{höhere Programmiersprache} und bei $L_2$ um \colorbold{Maschienensprache} handelt, ist es fast unerlässlich einen \colorbold{Pass} einzuführen, der \colorbold{komplexe Ausdrücke} (Definition~\ref{def:komplexer_ausdruck}) aus \colorbold{Operationen}\footnote{Hierunter fallen u.a. auch \colorbold{Zuweisungen}.} und je nach Programmiersprache aus \colorbold{weiteren Statements} \colorbold{entfernt}, sodass am Ende möglichst viele Statements \colorbold{Atomare Ausdrücke} (Definiton~\ref{def:atomarer_ausdruck}) als Argumente haben.
...
\begin{figure}[H]
  ...
  \hfill
  \codebox[title=Code in Monadischer Normalform, remember as=mon, width=0.3\textwidth, nobeforeafter, minted language=c]{./code_examples/example_remove_complex_operands_mon.special}

  \begin{tikzpicture}[overlay,remember picture,line width=1mm,draw=PrimaryColor]
  \draw[->] (code.east) to[bend right] node[below, align=center] {entferne \colorbold{Komplexe Audrücke}\\ aus \colorbol{Statements}} (mon.west);
  \end{tikzpicture}
