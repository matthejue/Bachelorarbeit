...
Hat die Zielmaschine $M_2$ allerdings ausreichend Rechenleistung, um Programme, die in $L_w$ geschrieben sind, selbst in ihre Maschienensprache $B_2$ zu kompilieren und es fehlt lediglich ein Compiler $C_w$, der das übernimmt, so stösst man, wenn man auf der Maschiene $M_2$ einen Compiler $C_w$ implementieren will und diesen nicht komplett in der Assemblersprache $A_2$ der Zielmaschine\footnote{Es wird davon ausgegangen, dass für die Maschiene $M_2$ zumindest ein \colorbold{Assembler} existiert.} implementieren will auf ein Problem, wenn man den Compiler $C_w$ in der Sprache $L_w$ schrieben will, die er selbst kompiliert.
...
\begin{Definition}{Bootstrapping}{bootstrapping}
  ...
  Wenn man die akutelle Version eines Compiler $C_{w_i}$ für die momentane Version einer Wunschsprache $L_{w_i}$ mithilfe von \colorbold{früheren Versionen} seiner selbst schreibt. Man schreibt den erweiterten Compiler in der Sprache $L_{w_{i-1}}$, welche von der früheren Version des Compilers, $C_{w_{i-1}}$ kompiliert wird und schafft es so iterativ immer umfangreichere Compiler zu bauen.\footcite{earley_formalism_1970}
