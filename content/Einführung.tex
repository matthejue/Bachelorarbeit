...
\begin{Definition}{Interpreter}{interpreter}
  ...
  Auf die Implementierung bezogen arbeitet ein Interpreter auf \colorbold{Sub-Bäumen} des \colorbold{Abstract Syntax Tree} und führt je nach Komposition der \colorbold{Nodes} des Abstract Syntax Tree, auf die er während des Darüber-Iterierens stösst unterschiedliche Anweisungen aus.
\end{Definition}
...
\begin{Definition}{Cross-Compiler}{cross_compiler}
  Kompiliert ein Program für eine Sprache, die auf dem eigenen System bzw. Rechner garnicht läuft.
  \footnote{Lego Mindstorms} % TODO:
\end{Definition}

  Ein \colorbold{Cross-Compiler} ist entweder notwendig, wenn es noch gar keinen Compiler der Wunschsprache für die \colorbold{Maschienensprache} eines Zielrechners gibt und man nicht den kompletten Compiler der Wunschsprache für den Zielrechner in der \colorbold{Maschienensprache} des Zielrechners schreiben will oder % TODO

  Daher ist eine mögliche Vorgehensweise einen \colorbold{minimalen Compiler} zu schreiben, der die \colorbold{Wunschsprache} in die \colorbold{Maschienensprache} des Zielrechners kompiliert und sich dafür die \colorbold{Rechenleistung} einer anderen Maschine borgt und eine beliebige \colorbold{höhere Sprache}, die auf dieser Maschien läuft, in der dieser Minimale Compiler geschrieben wird.

  Hat man diesen \colorbold{minimalen Compiler} kann man Programme in der Wunschsprache schreiben und mithilfe des  \colorbold{minimalen Compilers} kompilieren, sodass diese Programme auf dem Zielrechner laufen.

  Nun kann man den \colorbold{minimalen Compiler}, denn man gerade eben in einer \colorbold{höheren Programmesprache} implementiert hat, in der \colorbold{Wunschsprache} selbst implementieren und dann mit dem \colorbold{minamlen Compiler} für ebendiese Wunschsprache selbst kompilieren. Was man als Output bekommt ist ein \colorbold{minimmaler Compiler}, der aber auf dem \colorbold{Zielrechner} läuft und die \colorbold{Wunschsprache} in die \colorbold{Maschienensprache} des Zielrechners kompiliert.

  Aufbauend auf diesem \colorbold{minimalen Compiler}, der auf dem \colorbold{Zielrechner} läuft, kann man nun auf dem Zielrechner selbst iterativ den \colorbold{minimalen} Compiler schrittweise zu einem umfangreicheren Compiler, der mehr Funktionalitäten unterstützt weiterentwickeln und braucht den ursprünglichen Rechner, auf dem man die allererste Version des minimalen Compilers implementiert hat nicht mehr.
