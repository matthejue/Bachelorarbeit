...
Die \colorbold{Syntax}, in welcher der \colorbold{Inputstring} aufgeschrieben ist, wird auch als \colorbold{Konkrette Syntax} (Definition \ref{def:konkrette_syntax}) bezeichnet. Zum Schluss der \colorbold{Syntaktischen Analyse} ist der Inputstring in  \colorbold{Abstrakte Syntax} (Definition \ref{def:abstrakte_syntax}) übersetzt.
...
\begin{Definition}{Derivation Tree (bzw. Parse Tree)}{derivation_tree}
  \colorbold{Compilerinterne Darstellung} eines Programs in  als Baumdatenstruktur, in der \colorbold(Nichtterminalsybmole) die \colorbold{Inneren Knoten} des Baumes bilden und \colorbold{Terminalsymbole} die \colorbold{Blätter} des Baumes bilden.


  , \colorbold(welches) aus einem \colorbold{Inputstring}, der in \colorbold{Konkretter Syntax} angegeben ist generiert wurde, welche

Form in Compiler.

Inside the compiler, we
use abstract syntax trees (ASTs) to represent programs in a way that efficiently
supports the operations that the compiler needs to perform
\end{Definition}
