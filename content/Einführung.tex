...
\begin{itemize}[itemsep=-1mm, topsep=-1mm]
  \coloritem[Top-Down Parsing] Die Struktur dieser \colorbold{Parser} ist sehr nah an der \colorbold{Konkretten Syntax}, deren Sprache sie erkennen sollen. Es wird versucht durch Expansion der Produktionen der \colorbold{Konkretten Syntax} bis zu den Blättern, \colorbold{Linksableitungen} zu finden, sodass die \colorbold{Terminalsymbole} der \colorbold{Blätter} mit dem \colorbold{Inputstring} matchen\footnote{\colorbold{Rechtsableitungen} wären mit \colorbold{Recursive-Descent} auch möglich, wenn man dann auch über den Inputstring \colorbold{rückwärts} iteriert. Es gibt allerdings keinen guten Grund, warum man immer das \colorbold{rechteste} \colorbold{Non-Terminal} zuerst expandieren sollte, da für \colorbold{Kontextfreie Grammatiken} sowohl mit \colorbold{Links- als auch Rechtsableitungen} alle Wörter der Sprache, die die \colorbold{Kontextfreie Grammatik} beschreibt, ableitbar sind.}. Die richtigen \colorbold{Linksableitungen} für die \colorbold{Non-Terminale} der \colorbold{Konkretten Syntax}, welche oftmals nach den Regeln der Konkretten Syntax \colorbold{Nicht-deterministisch} mehrere Möglichkeiten der Expansion haben, werden entweder durch \colorbold{Backtracking} oder \colorbold{Vorausschauen} gefunden bzw. korrekt gewählt.

  Test
  \begin{itemize}[itemsep=-1mm, topsep=-1mm]
    \coloritem[Recursive-Descent / Backtracking Parser]
    \coloritem \footnote{Diese Art von Parser wurde im \colorbold{PicoC-Compiler} implementiert, als dieser noch auf dem Stand des \colorbold{Bachelorprojektes} war, bevor er durch den nicht selbst implementierten \colorbold{Earlay Parser} von Lark ersetzt wurde.}
    \coloritem \footnote{Diese Art von Parser ist im \colorbold{RETI-Interpreter} implementiert, da die \colorbold{RETI-Sprache} eine besonders simple \colorbold{LL(1) Grammatik} besitzt.}
    \coloritem \footnote{Das wird auch als \colorbold{Lookahead} von $k$ bezeichnet.}
  \coloritem[LL Parser] Diese Parser erkennen \colorbold{LL(k) Grammatiken}. Wobei eine Grammatik \colorbold{LL(k)} ist, wenn ein \colorbold{LL(k) Recognizer} dafür konstruiert werden kann, der diese erkennt. Dabei steht \colorbold{LL} für \colorbold{l}eft-to-right und \colorbold{l}eftmost-derivation, da die \colorbold{Token} von \colorbold{links nach rechts} gematched werden und immer das \colorbold{linkeste Non-Terminal} expandiert wird. Das $k$ steht dafür, dass dieser \colorbold{Parser}, um eine konstante Zahl $k$ von \colorbold{Tokens} vorausschauen kann. Diese Parser sind nicht in der Lage mit \colorbold{Linksrekursive} Grammatiken TODO
  \end{itemize}
  \coloritem[Bottom-Up Parsing] Es wird mit dem Inputstring gestartet und versucht, die \colorbold{Folge von Ableitungen}, die zu seiner Entstehung führt durch Rückwärtsanwenden der \colorbold{Produktionen} der \colorbold{Konkretten Syntax} bis zum  \colorbold{Startsymbol} zurückzuverfolgen.
\end{itemize}
