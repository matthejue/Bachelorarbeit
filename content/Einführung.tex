...
\begin{itemize}[itemsep=-1mm, topsep=-1mm]
  \coloritem[Top-Down Parsing]

  Es wird versucht durch Expansion der Produktionen der \colorbold{Konkretten Syntax} bis zu den Blättern, \colorbold{Linksableitungen} zu finden, sodass die \colorbold{Terminalsymbole} der \colorbold{Blätter} mit dem \colorbold{Inputstring} matchen.
%
  Die richtigen \colorbold{Linksableitungen} für die \colorbold{Non-Terminale} der \colorbold{Konkretten Syntax}, welche oftmals nach den Regeln der Konkretten Syntax \colorbold{Nicht-deterministisch} mehrere mögliche Ableitungen haben, werden entweder durch \colorbold{Backtracking} oder \colorbold{Vorausschauen} gefunden bzw. korrekt gewählt.

  % -------------------------------------------------------------------------------

  Eine sehr einfach zu implementierende Technik für \colorbold{Top-Down Parser} ist hierbei der \colorbold{Rekursive Abstieg}. Dabei wird jedem \colorbold{Nicht-Terminalsymbol} eine \colorbold{Prozedur} zugeordnet, welche die Produktionsregeln dieses \colorbold{Nicht-Terminalsymbols} umsetzt. Prozeduren rufen sich dabei wechselseitig gegenseitig entsprechend der \colorbold{Produktionsregeln} auf, falls eine entsprechende Produktionsregel eine \colorbold{Rekursion} enthält.\footnote{Diese Art von Parser wurde im \colorbold{PicoC-Compiler} implementiert, als dieser noch auf dem Stand des \colorbold{Bachelorprojektes} war, bevor er durch den nicht selbst implementierten \colorbold{Earley Parser} von Lark ersetzt wurde.}

  \colorbold{Rekursiver Abstieg} kann mit \colorbold{Backtracking} verbunden werden, um auch Grammatiken parsen zu können, die nicht \colorbold{LL(k)} (Definition \ref{def:llk_grammatik}) sind.


  \footnote{Diese Art von Parser ist im \colorbold{RETI-Interpreter} implementiert, da die \colorbold{RETI-Sprache} eine besonders simple \colorbold{LL(1) Grammatik} besitzt.}


  \coloritem[Bottom-Up Parsing] Es wird mit dem Inputstring gestartet und versucht, die \colorbold{Folge von Ableitungen}, die zu seiner Entstehung führt durch Rückwärtsanwenden der \colorbold{Produktionen} der \colorbold{Konkretten Syntax} bis zum  \colorbold{Startsymbol} zurückzuverfolgen.\footnote{Unter dieser Kategorie fällt der \colorbold{Eargg}}
\end{itemize}
