...
Üblicherweise kompiliert ein \colorbold{Compiler} von einer \colorbold{komplexeren Sprache} zu \colorbold{Maschinensprache} (Definition~\ref{def:maschienensprache}), aber es gibt z.B. auch \colorbold{Transpiler} (Definition~\ref{def:transpiler}) oder \colorbold{Cross-Compiler} (Definition~\ref{def:cross_compiler}).
...
\end{Definition}
  ...
  Diesen \colorbold{minimalen Compiler} $C_{w_{min}}$ könnte man nun auf dem aufwendigen Weg komplett in der \colorbold{Assemblersprache} $A_2$ (Definition~\ref{def:assemblersprache}) der \colorbold{Maschienensprache} $M_2$ schreiben oder man verwendet einen anderen Compiler $C_o$, der eine Sprache $L_o$ kompiliert und schreibt diesen \colorbold{minimalen Compiler} $C_{w_{min}}$ in dieser Sprache $L_o$ und kompiliert diesen dann mit dem Compiler $C_o$. Dieser andere Compiler $C_o$ kann entweder ein \colorbold{Cross-Compiler} sein, der auf einer anderen Maschine $M_1$ läuft oder ein Compiler, der bereits auf der Zielmaschine $M_2$ mittels \colorbold{Bootstrapping} (Definition~\ref{def:bootstrapping}) zum laufen gebracht wurde.

  \begin{Definition}{Assemblersprache (bzw. engl. Assembly Language))}{assemblersprache}
    Eine sehr \colorbold{hardwarenahe} Programmiersprache, derren \colorbold{Instructions} eine starke Entsprechung zu bestimmten Maschienenbefehlen bzw. Folgen von Maschienenbefehlen\footnote{Solche \colorbold{Instructions} der Assemblersprache werden auch als \colorbold{Pseudoinstructions} bezeichnet und entsprechen dem, was man im allgemeinen als \colorbold{Macro} bezeichnet.} haben.
    Viele \colorbold{Instructions} haben einen ähnliche Struktur mit \verb|Operation <Operandenliste>|, mit einer \colorbold{Operation} und keinen oder mehreren \colorbold{Operanden}, wie die späteren Maschienenbefehle, denen sie entsprechen. Allerdings gibt es oftmals noch viel \glqq syntaktischen Zucker\grqq\; innerhalb\footnote{Z.B. erlaubt die Assemblersprache des \colorbold{GCC} für die \colorbold{$X_{86\_64}$-Architektur} für manche Operanden die Syntax \smalltt{n(\%r)}, die einen \colorbold{Speicherzugriff} mit \colorbold{Offset} $n$ zur Adresse, die im \colorbold{Register} \smalltt{\%r} steht durchführt, wobei z.B. die Klammern \smalltt{()} usw. nur syntaktischer Zucker sind und natürlich nicht mitcodiert werden.} der Instructions und drumherum\footnote{Z.B. sind in $X_{86\_64}$ die Instructions in Blöcken untergebracht, die einen Bezeichner haben und zu denen mittels  \smalltt{jmp <blockbezeichner>} gesprungen werden kann, wobei ein solches Konstrukt, was vor allem auch noch beliebige Bezeichner verwendet keine Entsprechung in Prozessor und Hauptspeicher hat} eingefügt.
  \end{Definition}
  % TODO: die Sache mit Blocks und GOTO in x_86, aber eher später

  \begin{Definition}{Maschienensprache}{maschienensprache}
    Programmiersprache, deren Programme die \colorbold{hardwarenaheste Repräsentation} eines möglicherweise zuvor hierzu kompilierten bzw. assemblierten Programmes darstellen. Jede Instruction entspricht einer bestimmten \colorbold{Aufgabe}, die CPU übernehmen kann und die Instructions sind meist so designed, dass sie sich innerhalb bestimmter \colorbold{Wortbreiten}, die $2$er Potenzen sind codieren lassen. Im einfachsten Fall innerhalb einer \colorbold{Speicherzelle} des \colorbold{Hauptspeichers}.\footnote{Viele Prozessorarchitekturen erlauben es allerdings auch z.B. zwei Instructions in eine Speicherzelle des Hauptspeichers zu komprimieren, wenn diese zwei Instructions keine Operanden mit zu großen Immediate Values (Definition~\ref{def:immediate_value}) haben oder eine Instruction über mehrere Speicherzellen.}
  \end{Definition}

  \begin{Definition}{Assembler}{assembler}
    Assembliert \colorbold{Assemblersprache} in \colorbold{Maschienensprache}.
  \end{Definition}
