...
\begin{Definition}{Mehrdeutige Grammatik}{mehrdeutige_grammatik}
  \enquote{Eine Grammatik ist \colorbold{mehrdeutig}, wenn es ein Wort $w\in L(G)$ gibt, das mehrere \colorbold{Ableitungsbäume} zulässt}.\footnote{Alternativ, wenn es für $w$ \colorbold{mehrere} unterschiedliche \colorbold{Linksableitungen} gibt.}
  \footcite{nebel_theoretische_2020}
% TODO: (Bild hierfür)
\end{Definition}
...
Will man die \colorbold{Operatoren} aus einer \colorbold{Programmiersprache} in einer \colorbold{Grammatik} für eine \colorbold{Konkrette Syntax} ausdrücken, die \colorbold{nicht} \colorbold{Mehrdeutig} ist, so lässt sich das nach einem klaren Schema machen, wenn die \colorbold{Assoziativität} (Definiton~\ref{def:assoziativität}) und \colorbold{Präzidenz} (Definition~\ref{def:präzidenz}) dieser \colorbold{Operatoren} festgelegt ist. Dieses Schema wird in Unterkapitel~\ref{sec:umsetzung_von_präzidenz} genauer erklärt.

\begin{Definition}{Assoziativität}{assoziativität}
  \enquote{Bestimmt, welcher Operator aus einer Reihe \colorbold{gleicher} Operatoren \colorbold{zuerst} ausgewertet wird.}

  Es wird grundsätzlich zwischen \colorbold{linksassoziativen} Operatoren, bei denen der \colorbold{linke Operator} vor dem \colorbold{rechten Operator} ausgewertet wird und \colorbold{rechtsassoziativen} Operatoren, bei denen es genau anders rum ist unterschieden.
  \footcite{noauthor_parsing_nodate}
\end{Definition}

Bei \colorbold{Assoziativität} ist z.B. der \colorbold{Multitplikationsoperator} \smalltt{*} ein Beispiel für einen \colorbold{linksassoziativen} Operator und ein \colorbold{Zuweisungsoperator} \smalltt{=} ein Beispiel für einen \colorbold{rechtsassoziativen} Operator. Dies ist in Abbildung~\ref{fig:linksassoziativität_rechtsassoziativität_veranschaulichung} mithilfe von Klammern \smalltt{()} veranschaulicht.

\begin{figure}[H]
  \hspace{2cm}
  \codebox[title=Ohne Klammern, remember as=linksassoziativohneklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/left_assoziatice_code.special}
  \hfill
  \codebox[title=Mit Klammern, remember as=linksassoziativmitklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/left_assoziatice_parens.special}
  \hspace{2cm}

  \vspace{1cm}

  \hspace{2cm}
  \codebox[title=Ohne Klammern, remember as=rechtsassoziativohneklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/right_assoziatice_code.special}
  \hfill
  \codebox[title=Mit Klammern, remember as=rechtsassoziativmitklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/right_assoziatice_parens.special}
  \hspace{2cm}

  \begin{tikzpicture}[overlay,remember picture,line width=1mm,draw=PrimaryColor]
  \draw[->] (linksassoziativohneklammern.east) to[bend left] node[above] {Linksassoziativität} (linksassoziativmitklammern.west);
  \draw[->] (rechtsassoziativohneklammern.east) to[bend right] node[below] {Rechtsassoziativität} (rechtsassoziativmitklammern.west);
  \end{tikzpicture}
  \caption{Veranschaulichung von Linksassoziativität und Rechtsassoziativität}
  \label{fig:linksassoziativität_rechtsassoziativität_veranschaulichung}
\end{figure}

\begin{Definition}{Präzidenz}{präzidenz}
  \enquote{Bestimmt, welcher Operator \colorbold{zuerst} in einem Ausdruck, der eine Mischung \colorbold{verschiedener} Operatoren enthält, ausgewertet wird. Operatoren mit einer \colorbold{höheren Präzidenz}, werden \colorbold{vor} Operatoren mit \colorbold{niedrigerer Präzidenz} ausgewertet.}
  \footcite{noauthor_parsing_nodate}
\end{Definition}

Bei \colorbold{Präzidenz} ist die Mischung der Operatoren für \colorbold{Subraktion} \smalltt{'-'} und für \colorbold{Multiplikation} \smalltt{*} ein Beispiel für den Einfluss von Präzidenz. Dies ist in Abbildung~\ref{fig:precedence_veranschaulichung} mithilfe der Klammern \smalltt{()} veranschaulicht. Im Beispiel in Abbildung~\ref{fig:precedence_veranschaulichung} ist bei den beiden \colorbold{Subtraktionsoperatoren} \smalltt{'-'} nacheinander und dem darauffolgenden \colorbold{Multitplikationsoperator} \smalltt{*} sowohl \colorbold{Assoziativität} als auch \colorbold{Präzidenz} im Spiel.

\begin{figure}[H]
  \hspace{2cm}
  \codebox[title=Ohne Klammern, remember as=precidenceohneklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/precidence_code.special}
  \hfill
  \codebox[title=Mit Klammern, remember as=precidencemitklammern, width=4cm, nobeforeafter, minted language=c]{./code_examples/precidence_parens.special}
  \hspace{2cm}

  \begin{tikzpicture}[overlay,remember picture,line width=1mm,draw=PrimaryColor]
  \draw[->] (precidenceohneklammern.east) to[bend right] node[above] {Präzidenz} (precidencemitklammern.west);
  \end{tikzpicture}
  \caption{Veranschaulichung von Präzidenz}
  \label{fig:precedence_veranschaulichung}
\end{figure}
...
Nachdem alle \colorbold{Passes} durchgearbeitet wurden, ist es notwendig aus dem finalen \colorbold{Abstract Syntax Tree} den eigentlichen \colorbold{Maschinencode} in \colorbold{Konkretter Syntax} zu generieren. In üblichen Compilern wird hier für den \colorbold{Maschinencode} eine \colorbold{binäre Repräsentation} gewählt\footnote{Da der \colorbold{PicoC-Compiler} vor allem zu \colorbold{Lernzwecken} konzipiert ist, wird bei diesem der \colorbold{Maschienencode} allerdings in einer \colorbold{menschenlesbaren Repräsentation} ausgegeben.}. Der Weg von \colorbold{Abstrakter Syntax} zu  \colorbold{Konkretter Syntax} ist allerdings wesentlich einfacher, als der Weg von der \colorbold{Konkretten Syntax} zur \colorbold{Abstrakten Syntax}, für die eine gesamte \colorbold{Syntaktische Analyse}, die eine \colorbold{Lexikalische Analyse} beinhaltet durchlaufen werden musste.

Jeder \colorbold{Knoten} des \colorbold{Abstract Syntax Trees} erhält dazu eine Methode, welche hier \smalltt{to\_string} genannt wird, die eine \colorbold{Textrepräsentation} seiner selbst und all seiner Knoten mit an den richtigen Stellen passend gesetzten \colorbold{Semikolons} \smalltt{;} usw. ausgibt. Dabei wird nach dem \colorbold{Depth-First-Search} Schema der gesamte \colorbold{Abstract Sybtax Tree} durchlaufen und die Methode \smalltt{to\_string} zur Ausgabe der \colorbold{Textrepräsentation} der verschiedenen Knoten aufgerufen, die immer wiederum die Methode \smalltt{to\_string} ihrer Kinder aufrufen und die zurückgegebene Textrepräsentation passend \colorbold{zusammenfügen} und selbst \colorbold{zurückgebeben}.
