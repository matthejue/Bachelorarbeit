...
Üblicherweise kompiliert ein \colorbold{Compiler} ein Program, dass in einer \colorbold{Programmiersprache} geschrieben ist zu \colorbold{Maschienenncode}, der in \colorbold{Maschienensprache} (Definition~\ref{def:maschienensprache}) geschrieben ist, aber es gibt z.B. auch \colorbold{Transpiler} (Definition~\ref{def:transpiler}) oder \colorbold{Cross-Compiler} (Definition~\ref{def:cross_compiler}).
...
Der \colorbold{PicoC-Compiler}, der den Zweck erfüllt für Studenten ein \colorbold{Anschauungs- und Lernwerkzeug} zu sein, generiert allerdings \colorbold{Maschienencode}, der die \colorbold{Maschienenbefehle} bzw. \colorbold{RETI-Befehle} in menschenlesbarer Form als, ausgeschriebene Bezeichnungen der \colorbold{RETI-Operationen}, \colorbold{RETI-Register} und \colorbold{Immediates} (Definition~\ref{def:immediate_value}) enthält. Der Maschienencode des \colorbold{PicoC-Compilers} braucht die Maschienenbefehle auch nicht in binärer Darstellung, damit der \colorbold{RETI-Interpreter} sie interpretieren kann, sondern es für den \colorbold{RETI-Interpreter} ebenfalls leichter diese direkt in menschenlesbarer Form zu haben, da der \colorbold{RETI-Interpreter} einen echten RETI-Prozessor, der so noch nicht existiert nur simuliert\footnote{Einen \colorbold{RETI-Prozessor} zu bauen, der menscnenlesbaren Maschienencode in z.B. \colorbold{UTF-8 Codierung} implementiert wäre vermutlich sehr kompliziert und unnötiger Aufwand, da Hardware binär arbeitet und man dieser daher lieber direkt die binär codierten Maschiendenbefehle übergibt, anstatt z.B. eine unnötig Platzverbrauchenden \colorbold{UTF-8 Codierung} zu verwenden, die man nicht in einem Schritt verarbeiten kann, da die Register und Speicherzellen nur 32 bzw. 64 Bit Breite haben}.
