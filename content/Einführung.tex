...
aus \colorbold{Operationen}\footnote{\colorbold{Statement} oder \colorbold{Ausdruck} der einem \colorbold{Maschienenbefehl} entspricht und z.B. auf der \colorbold{RETI-CPU} in einem \colorbold{Fetch- und Execute-Zyklus} ausgeführt werden kann.}\footnote{Hierunter fallen u.a. \colorbold{Zuweisungen} \smalltt{var = 12} und \colorbold{Ausdrücke}, wie \smalltt{12 \% 5}.} und je nach Programmiersprache aus den \colorbold{Teilausdrücken} weiterer \colorbold{Statements} und \colorbold{Ausdrücke} entfernt, die nicht direkt einer \colorbold{Operation} entsprechen\footnote{Z.B das \colorbold{Statement} \smalltt{if(<condition>)\,\{<stmts>\}\, else\,\{<stmts>\}}.} und in \colorbold{späteren Passes} erst zu einer \colorbold{Folge} von \colorbold{Maschienenbefehlen} umgewandelt werden.

Auf diese Weise haben am Ende alle \colorbold{Statements} nur \colorbold{Atomare Ausdrücke} (Definiton~\ref{def:atomarer_ausdruck}) als Teilausdrücke.
...
\begin{Definition}{Atomarer Ausdruck}{atomarer_ausdruck}
  Ein \colorbold{Atomarer Ausdruck} ist ein Ausdruck, der \colorbold{atomar} ist, also \colorbold{nicht} mehr weiter \colorbold{zerkleinert} werden kann. Also entweder eine \colorbold{Variable} \smalltt{var}, eine \colorbold{Zahl} \smalltt{12} oder ein \colorbold{ASCII-Zeichen} \smalltt{'c'} ist.\footcite{g_siek_course_2022}
\end{Definition}

\begin{Definition}{Komplexer Ausdruck}{komplexer_ausdruck}
  Ein \colorbold{komplexer Ausdruck} ist ein \colorbold{Ausdruck}, der \colorbold{nicht atomar} ist, wie z.B. \smalltt{5 \% 4}, \smalltt{-1} oder \smalltt{fun(12)}.\footcite{g_siek_course_2022}
\end{Definition}

% die Sache mit Variablen

% Die Sache Monadischer Normalform

\begin{Definition}{A-Normal Form}{a_normal_form}
  Eine Sprache bei der
  \footcite{noauthor_-normalization_nodate}
  \footcite{bolingbroke_types_2009}
\end{Definition}

Ein \colorbold{Beispiel} für dieses Vorgehen ist in Abbildung~\ref{fig:example_remove_complex_operands} zu sehen, wo der Einfachheit halber auf die Darstellung als Abstract Syntax Tree \colorbold{verzichtet} wurde und die Codebeispiele in \colorbold{Konkretter Syntax} aufgeschrieben wurden wurden.

\begin{figure}[H]
  \codebox[title=Code , remember as=code, width=0.3\textwidth, nobeforeafter, minted language=c]{./code_examples/example_remove_complex_operands_code.special}
  \hfill
  \codebox[title=Code in A-Normalform, remember as=mon, width=0.3\textwidth, nobeforeafter, minted language=c]{./code_examples/example_remove_complex_operands_mon.special}

  \begin{tikzpicture}[overlay,remember picture,line width=1mm,draw=PrimaryColor]
  \draw[->] (code.east) to[bend right] node[below, align=center] {entferne \colorbold{Komplexe Audrücke}\\ aus \colorbold{Statements}.} (mon.west);
  \end{tikzpicture}
...
Dieser \colorbold{Pass} hat vor allem die Aufgabe sich der \colorbold{Syntax} von \colorbold{Maschienenbefehlen} besonders dadurch anzunähren, dass mit \colorbold{Ausnahme} einiger \colorbold{spezieller Statements}, die in späteren Passes umgwandelt werden müssen, so weit wie möglich immer nur \colorbold{eine} Operation pro \colorbold{Statement} ausgeführt wird. Dadurch lassen sich viele Statements bereits direkt zu \colorbold{Maschienenbefehlen} übersetzen, da Machienenbefehle üblicherweise immer nur \colorbold{eine} Operation ausführen können\footnote{Z.B. kann eine \colorbold{RETI-CPU} in einem \colorbold{Fetch- und Execute-Zyklus} nur eine Operation mit der \colorbold{Arithmetisch Logischen Einheit} (engl. ALU) ausführen.}.
