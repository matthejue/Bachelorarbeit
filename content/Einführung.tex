...
\begin{Definition}{Lexer (bzw. Scanner)}{lexer}
  Ein \colorbold{Lexer} ist eine \colorbold{partielle} Funktion \hspace{0.2cm}$lex: \sum^{*} \rightharpoonup (N \times W)^{*}$, welche ein \colorbold{Wort} aus $\sum^{*}$ auf ein \colorbold{Token} $T$ mit einem \colorbold{Tokennamen} $N$ und einem \colorbold{Tokenwert} $W$ abbildet, falls diese Folge von Symbolen sich unter der \colorbold{regulären Grammatik} ${G}_{Lex}$, der \colorbold{regulären Sprache} ${L_{Lex}}$ abbleiten lässt.\footcite{noauthor_lecture-notes-2021_2022}
\end{Definition}
...
% weil es z.B. unter anderem oftmals viele verschiedene \colorbold{Literale} (Definition \ref{def:literal}) für einen Zahlenwert gibt und und

% Definition Literal
% VAlue und LExeme, wegen Literal nennt man es nicht so
...
% Fehlermeldung bei Lexer, wenn z.B. ein ubekanntes Zeichen auftaucht. Meist sind aber alle möglichen Zeichenkombination allein dadruch schon möglich, dass es einen Identifier gibt, der alle möglichen Zeichenfolgen abfängt. Wodurch der Lexer wiederum doch eine linkstotale Funktion ist, die man im Allgemeinen einfach als Funktion bezeichnet.
