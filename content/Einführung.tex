...
Im Falle dessen, dass es sich bei der \colorbold{Sprache} $L_1$ um eine \colorbold{höhere Programmiersprache} und bei $L_2$ um \colorbold{Maschienensprache} handelt, ist es fast unerlässlich einen \colorbold{Pass} einzuführen, der \colorbold{komplexe Ausdrücke} (Definition~\ref{def:komplexer_ausdruck}) aus \colorbold{Operationen}\footnote{Hierunter fallen u.a. auch Zuweisungen.} und je nach Programmiersprache aus \colorbold{weiteren Statements} \colorbold{entfernt}, sodass am Ende alle Statements \colorbold{Atomare Ausdrücke} (Definiton~\ref{def:atomarer_ausdruck}) als Argumente haben.

Dieser \colorbold{Pass} hat vor allem die Aufgabe sich der \colorbold{Syntax} von \colorbold{Maschienenbefehlen} besonders dadurch anzunähren, dass mit Ausnahme einiger spezieller Statements, die in späteren Passes umgwandelt werden müssen, so weit wie möglich immer nur eine Operation pro Statement ausgeführt wird. Dadurch lassen sich viele Statements bereits direkt zu Maschienenbefehlen übersetzen, da Machienenbefehle üblicherweise immer nur für eine Operation ausführen können\footnote{Z.B. kann eine \colorbold{RETI-CPU} in einem \colorbold{Fetch- und Execute-Zyklus} nur eine Operation mit der \colorbold{Arithmetisch Logischen Einheit} (engl. ALU) ausführen.}.

% den \colorbold{Bedingungen} \smalltt{<condition>} von \smalltt{if(<condition>){<stmts>}else{<stmts>}}-Statements entfernt


wie in dem Beispiel in Abbildung~\ref{} zu sehen ist.

% ähnlicher zu Maschieneninstrucitns, die nur eine Operation auf einmal
