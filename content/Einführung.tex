...
Hat die Zielmaschine $M_2$ allerdings ausreichend Rechenleistung, um Programme, die in $L_w$ geschrieben sind, selbst in ihre Maschienensprache $B_2$ zu kompilieren und es fehlt lediglich ein Compiler $C_w$, der das übernimmt, so ist eine mögliche Lösung, um diesen \colorbold{Compiler} $C_w$ zu erhalten, in einem ersten Schritt einen \colorbold{minimalen Compiler} $C_{w_{min}}$ zu schreiben, der eine reduzierte \colorbold{Wunschsprache} $L_{w_{min}}$ in die \colorbold{Maschienensprache} $B_2$ der Zielmaschine $M_2$ kompiliert.
