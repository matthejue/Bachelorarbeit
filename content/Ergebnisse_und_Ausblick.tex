...
Sollte tatsächlich eines Tages eine \colorbold{RETI-CPU} in Hardware implementiert worden sein und auf einem \colorbold{FPGA} laufen, sodass ein \colorbold{provisorisches Betriebssystem} darauf läuft, dann wäre der nächste Schritt einen PicoC-Compiler zu schreiben, der in der Programmiersprache PicoC, $L_{PicoC}$ selbst geschrieben ist und sich selbst kompilieren kann. Je nach Implementierungsart dieses \colorbold{Self-compiling Compiler} kann dadurch \colorbold{Unabhängigkeit} von der Programmiersprache \colorbold{Python}, in der er momentan implementiert ist und Unabhängigkeit von einer anderen Maschiene, die bisher immer für das \colorbold{Cross-Compiling} notwendig war erreicht werden.

Will man nun auf dem \colorbold{FPGA} $M_{FPGA}$ den gerade beschriebenen \colorbold{Self-compiling Compiler} $C_{PicoC\_PicoC}$ implementieren und hat bereits den gesamtem Compiler $C_{PicoC}$ in der Sprache  $L_{PicoC}$ geschrieben, so stösst man auf ein Problem, dass auf das \colorbold{Henne-Ei-Problem}\footnote{Beschreibt die Situation, wenn ein System sich selbst als Abhängigkeit hat, damit es überhaupt einen Anfang für dieses System geben kann. Dafür steht das Problem mit der Henne und dem Ei sinnbildlich, da hier die Frage ist, wie das ganze seinen Anfang genommen hat, da beides zirkular voneinander abhängt.} reduziert werden kann. Man bräuchte, um den Compiler $C_{PicoC}$ auf dem \colorbold{FPGA} $M_{FPGA}$ zu kompilieren bereits einen kompilierten Compiler $C_{PicoC}$, der mit der Maschienensprache \colorbold{RETI} läuft. Es liegt eine \colorbold{zirkulare Abhängigkeit vor}, die man nur auflösen kann, indem eine \colorbold{externe Entität} zur Hilfe nimmt.

Wenn man den gesamten \colorbold{Self-compiling Compiler} $C_{PicoC\_PicoC}$ nicht selbst komplett in der Maschienensprache \colorbold{RETI} schreiben will, wäre eine Möglichkeit, indem man den \colorbold{Cross-Compiler} $C_{PicoC\_PicoC}$, den man bereits in der Programmiersprache \colorbold{Python} implementiert hat auf einer anderen Maschiene $M$ dafür nutzt, dass dieser den \colorbold{Self-compiling Compiler} $C_{PicoC\_PicoC}$ für den FPGA $M_{FPGA}$ kompiliert bzw. \colorbold{bootstraped} (Definition~\nameref{par:bootstrapping1}). Im Fall, dass auf dem FPGA $M_{FPGA}$ die Programmiersprache $L_{Python}$ bereits mittels \colorbold{Bootstrapping} auf vielen denkbaren Wegen zum Laufen gebracht wurde, könnte der \colorbold{Self-compiling Compiler} $C_{PicoC\_PicoC}$ auch mithife des \colorbold{Cross-Compilers} $C_{PicoC\_Python}$ und der Programmiersprache \colorbold{Python} auf dem \colorbold{FPGA} selbst kompiliert werden.

Aufbauend auf dem \colorbold{Self-compiling Compiler} $C_{PicoC\_PicoC}$, der einen \colorbold{minimalen Compiler} für die \colorbold{Programmiersprache} $C$, $L_C$ darstellt, könnte man auch die komplette Sprache $L_C$ für den \colorbold{FPGA} $M_{FPGA}$ ebenfalls mittels \colorbold{Bootstrapping} (Defintion~\nameref{par:bootstrapping2}) implementieren.
