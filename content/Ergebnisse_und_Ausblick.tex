...
% Aufteilung der Tests, Hart zu jedem Großtema ein oder zwei harte Tests
  ...
  \begin{equation}
    \begin{tikzpicture}[auto, baseline=(current  bounding  box.center)]
      \node (reti) at (135:3) {$M_{1}$};
      \node (x_86) at (45:3) {$M_{2}$};
      \node (picoc) [above=of reti] {$T_1$};
      \node (c) [above=of x_86] {$T_2$};
      ...
      \draw[->] (picoc) to node[above] {convert\_to\_c} (reti);
      \draw[->] (picoc) to node[above] {convert\_to\_c} (c);
      \draw[->] (c) to node[above] {convert\_to\_c} (x_86);
      \draw[->] (reti) to[bend right] node[left, align=center] {interpretiere $P_{1}$\\ als $L_{1}$ Program} (output);
      \draw[->] (x_86) to[bend left] node[right, align=center] {interpretiere $P_{2}$\\ als $L_{2}$ Program} (output);
    \end{tikzpicture}
