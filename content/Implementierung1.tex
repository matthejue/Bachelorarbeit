...
\begin{table}[H]
  \center
  \begin{NiceTabular}{XX}[rules/color=PrimaryColor]
  \CodeBefore
  \rowcolor{PrimaryColor}{1}
  \rowcolors{2-13}{SecondaryColor}{}[cols={1-2}]
  \Body
  \textbf{\textcolor{white}{Komposition}} &	\textbf{\textcolor{white}{Beschreibung}} \\
  \smalltt{Ref(GlobalRead(addr))}	& Liest Speicherzelle aus, die \smalltt{addr} Speicherzellen \colorbold{relativ} zum Anfang der \colorbold{globalen Statischen Daten} liegt. \\
  \smalltt{Ref(StackRead(addr))} & Liest Speicherzelle aus, die \smalltt{addr} Speicherzellen \colorbold{relativ} zum Anfang des \colorbold{Stack} liegt. \\
  \smalltt{Ref(Subscr(Tmp(Num("x")), Tmp(Num("y"))))} & Berechnet die nächste \colorbold{Adresse} aus der \colorbold{Adresse}, die an Speicherzelle \smalltt{Tmp(Num('x'))} steht und dem \colorbold{Subscript Index}, der an Speicherzelle \smalltt{Tmp(Num('y'))} steht und speichert diese auf den Stack. \\
  \smalltt{Ref(Attr(Tmp(Num("1")), name))} & doof \\
  \smalltt{Ref(GlobalRead(num))} & \\
  \smalltt{Assign(Tmp(Num(str(size))), GlobalRead(num))} & \\
  \smalltt{Assign(Tmp(Num(str(size))), StackRead(num))} & \\
...
\begin{Special_Paragraph}
  ...
  Ersetzt ein \verb|exp|, dessen Wert bereits \colorbold{vorher} berechnet wurde und \smalltt{x} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer}  \smalltt{SP} steht bzw. wenn es sich um die \smalltt{main}-Funktion handelt  \colorbold{relativ} zum \colorbold{Pointer auf das Datensegment} \smalltt{DS}.
