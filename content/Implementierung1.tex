...
\begin{table}[H]
  ...
  \CodeBefore
  \rowcolor{PrimaryColor}{1}
  \rowcolors{2-43}{SecondaryColor}{}[cols={1-2}]
  \Body
  \textbf{\textcolor{white}{PiocC-Knoten}} &	\textbf{\textcolor{white}{Beschreibung}} \\
  \smalltt{Name()} & Steht für einen \colorbold{Identifier}, aber \smalltt{Name()}\\
  \smalltt{Num()} & \\
  \smalltt{Char()} & \\
  \smalltt{Minus()} & \\
  \smalltt{Not()} & \\
  ...
  \smalltt{CharType()} & \\
  \smalltt{VoidType()} & \\
  \smalltt{BinOp(exp1, bin\_op, exp2)} & \\
  \smalltt{UnOp(un\_op, exp)} & \\
  \smalltt{Exit(num)} & \\
  \smalltt{Atom(exp1, rel, exp2)} & \\
  \smalltt{ToBool(exp)} & \\
...
\begin{table}[H]
  ...
  \smalltt{FunDecl()} & \\
  \smalltt{FunDef()} & \\
  \smalltt{NewStackframe()} & \\
  \smalltt{RemoveStackframe()} & \\
  \smalltt{File()} & \\
  \smalltt{Block()} & \\
  \smalltt{GoTo()} & \\
  \smalltt{SingleLineComment()} & \\
  \smalltt{RETIComment()} & \\
  \smalltt{Placeholder()} & \\
...
\begin{Special_Paragraph}
  Die \textcolor{gray!90!black}{ausgegrauten} Attribute der PicoC-Nodes sind \colorbold{versteckte Attribute}, die nicht direkt bei der Erstellung der  \colorbold{PicoC-Nodes} mit einem Wert initialisiert werden, sondern im Verlauf der Kompilierung beim Durchlaufen der verschiedenen Passes ein verstecktes Attribut zugewiesen bekommen, dass im weiteren Kompiliervorgang Informationen transportiert, die später im Kompiliervorgang nicht mehr so leicht zugänglich wären.
