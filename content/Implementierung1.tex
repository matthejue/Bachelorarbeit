...
Hier sind jegliche \colorbold{Kompositionen} von \colorbold{PicoC-Nodes} und \colorbold{RETI-Nodes} aufgelistet, die eine \colorbold{besondere Bedeutung} haben und nicht bereits in der \colorbold{Abstrakten Syntax}~\ref{gr:concrete_syntax_parser} enthalten sind.
...
\begin{table}[H]
  ...
  \smalltt{Instr(Loadi(), [Reg(Acc()), Goto()])} & \\
  \bottomrule
\end{NiceTabular}
...
\begin{Special_Paragraph}
  Um die obige Tabelle~\ref{tab:kompositionen_von_picoc_nodes_und_reti_nodes_mit_besonderer_bedeutung} nicht mit unnötig viel repetetiven Inhalt zu füllen, wurden die zahlreichen Kompostionen \colorbold{ausgelassen}, bei denen einfach nur \verb|exp| durch $\mathtt{Tmp(Num('x')), x}\in\mathbb{N}$ ersetzt wurde.
  ...
  % TODO: in richtige Tabelle machen
  Ersetzt ein \verb|exp|, dessen Wert bereits \colorbold{vorher} berechnet wurde und \smalltt{x} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer} \smalltt{SP} steht.
