...
\begin{table}[H]
  ...
  \CodeBefore
  \rowcolor{PrimaryColor}{1}
  \rowcolors{2-10}{SecondaryColor}{}[cols={1-2}]
  \Body
  \textbf{\textcolor{white}{PiocC-Knoten}} & \textbf{\textcolor{white}{Beschreibung}} \\
  \smalltt{Name()} & Ein \colorbold{Bezeichner}, aber da es keine gute Kurzform für \smalltt{Identifier()} (englisches Wort für Bezeichner) gibt, wurde dieser Knoten \smalltt{Name()} genannt. \\
  \smalltt{Num()} & Eine \colorbold{Zahl}. \\
  \smalltt{Char()} & Ein \colorbold{Zeichen} der \colorbold{ASCII-Zeichenkodierung} . \\
  \smalltt{Minus(), Not(), DerefOp(), RefOp(), LogicNot()} & Die \colorbold{unären Operatoren} \smalltt{un\_op}: \smalltt{-a, {$\thicksim$}a, *a, \&a}. \\
  ...
  \smalltt{IntType(), CharType(), VoidType()} & \colorbold{Type Specifier für Primitiven Datentypen}, die in der Abstrakten Syntax, um eine intuitive Bezeichnung zu haben einfach nur unter \colorbold{Datentypen} eingeordnet werden: \smalltt{int},  \smalltt{char},  \smalltt{void}. \\
  \smalltt{BinOp(exp1, bin\_op, exp2)} & Container für eine \colorbold{binäre Operation}. \\
  \smalltt{UnOp(un\_op, exp)} & Container für eine \colorbold{unäre Operation}. \\
  \smalltt{Exit(num)} & \colorbold{Beendigung} des Programmes und ist Container für einen \colorbold{Exit Code}, der vor der Beendigung in das \smalltt{ACC} Register geschrieben wird. \\
  \smalltt{Atom(exp1, rel, exp2)} & Container für eine \colorbold{binäre Relation} und 2 \colorbold{Expressions}. \\
  \smalltt{ToBool(exp)} &  \\
  \smalltt{Alloc(type\_qual, datatype, name, \textcolor{gray!90!black}{local\_var\_or\_param})} & \\
  \smalltt{Assign(lhs, exp)} & \\
  \smalltt{Exp(exp, \textcolor{gray!90!black}{datatype}, \textcolor{gray!90!black}{error\_data})} & \\
  \smalltt{Tmp(num)} & \\
  \smalltt{StackRead(num)} & \\
  \smalltt{StackWrite(num)} & \\
  \smalltt{GlobalRead(num)} & \\
  \smalltt{GlobalWrite(num)} & \\
  \smalltt{StackMalloc(num)} & \\
  \bottomrule
\end{NiceTabular}
...
\begin{table}[H]
  ...
  \CodeBefore
  \rowcolor{PrimaryColor}{1}
  \rowcolors{2-27}{SecondaryColor}{}[cols={1-2}]
  \Body
  \textbf{\textcolor{white}{PiocC-Knoten}} & \textbf{\textcolor{white}{Beschreibung}} \\
  \smalltt{PntrDecl(num, \textcolor{gray!90!black}{datatype})} & \\
  \smalltt{Ref(exp, \textcolor{gray!90!black}{datatype}, \textcolor{gray!90!black}{error\_data})} & \\
  \smalltt{Deref(lhs, exp)} & \\
  \smalltt{ArrayDecl(nums, \textcolor{gray!90!black}{datatype})} & \\
  \smalltt{Array(exps, \textcolor{gray!90!black}{datatype})} & \\
  \smalltt{Subscr(lhs, exp)} & \\
  \smalltt{StructSpec(name)} & \\
  \smalltt{Attr(lhs, name)} & \\
  \smalltt{Struct(assigns, \textcolor{gray!90!black}{datatype})} & \\
  \smalltt{StructDecl(name, allocs)} & \\
  \smalltt{If(exp, stmts\_goto)} & \\
  \smalltt{IfElse(exp, stmts\_goto1, stmts\_goto2)} & \\
  \smalltt{While(exp, stmts\_goto)} & \\
  \smalltt{DoWhile(exp, stmts\_goto)} & \\
  \smalltt{Call(name, exps)} & \\
  \smalltt{Return(exp)} & \\
  \smalltt{FunDecl(datatype, name, allocs)} & \\
  \smalltt{FunDef(datatype, name, alocs, stmts\_blocks)} & \\
  \smalltt{NewStackframe(fun\_name, goto\_after\_call)} & \\
  \smalltt{RemoveStackframe()} & \\
  \smalltt{File(name, decls\_defs\_blocks)} & \\
  \smalltt{Block(name, stmts\_instrs, instrs\_before, num\_instrs, signature\_size, local\_vars\_size)} & \\
  \smalltt{GoTo(name)} & \\
  \smalltt{SingleLineComment(prefix, content)} & \\
  \smalltt{RETIComment()} & \\
  \smalltt{Placeholder()} & \\
  \smalltt{Tmp(Num('addr'))} & Steht für ein \verb|exp|, dessen Wert bereits \colorbold{vorher} berechnet wurde und \smalltt{x} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer} \smalltt{SP} steht. \\
  \bottomrule
\end{NiceTabular}
...
\begin{Special_Paragraph}
  Die \textcolor{gray!90!black}{ausgegrauten} Attribute der PicoC-Nodes sind \colorbold{versteckte Attribute}, die \colorbold{nicht} direkt bei der Erstellung der  \colorbold{PicoC-Nodes} mit einem Wert \colorbold{initialisiert} werden, sondern im \colorbold{Verlauf der Kompilierung} beim Durchlaufen der verschiedenen Passes ein verstecktes Attribut zugewiesen bekommen, dass im weiteren Kompiliervorgang \colorbold{Informationen} transportiert, die später im Kompiliervorgang nicht mehr so leicht zugänglich wären.
...
\begin{Definition}{Token-Knoten}{token_knoten}
  Ersetzt ein \colorbold{Token} bei der Generierung des \colorbold{Abstract Syntax Tree}, damit der Zugriff auf Knoten des Abstract Syntax Tree möglichst \colorbold{simpel} ist und keine vermeidbaren Fallunterscheidungen berücksichtigt werden müssen.
...
\begin{Definition}{Container-Knoten}{container_knoten}
  Dient als \colorbold{Container} für andere \colorbold{Container-Knoten} und \colorbold{Token-Knoten}. Die \colorbold{Container Knoten} werden optimalerweise immer so gewählt, dass sie mehrere \colorbold{Produktionen} der \colorbold{Konkretten} Syntax abdecken, die einen ähnlichen Aufbau haben und auch irgendeine Form der \colorbold{Semantischen Verwandschafft} vorliegt\footnote{Wie z.B. die verschiedenen Arithmetischen und Logischen Ausdrücke, die immer eine \colorbold{Operation} in der Mitte haben und $2$ \colorbold{Operanden} auf beiden Seiten.}.
...
\begin{table}[H]
  ...
  \CodeBefore
  \rowcolor{PrimaryColor}{1}
  \rowcolors{2-2}{SecondaryColor}{}[cols={1-2}]
  \Body
...
\begin{table}[H]
  ...
  \smalltt{Ref(GlobalRead(Num('addr')))}	& Speichert \colorbold{Adresse} der Speicherzelle, die \smalltt{Num('addr')} Speicherzellen \colorbold{relativ} zum \colorbold{Datensegment Register} \smalltt{DS} steht auf den \colorbold{Stack}. \\
  \smalltt{Ref(StackRead(Num('addr')))} & Speichert \colorbold{Adresse} der Speicherzelle, die \smalltt{Num('addr')} Speicherzellen \colorbold{relativ} zum \colorbold{Begin-Aktive-Funktion Register} \smalltt{BAF} steht auf den \colorbold{Stack}. \\
  \smalltt{Ref(Subscr(Tmp(Num('addr1')), Tmp(Num('addr2'))))} & Berechnet die nächste \colorbold{Adresse} aus der \colorbold{Adresse}, die an Speicherzelle \smalltt{Tmp(Num('addr1'))} steht und dem \colorbold{Subscript Index}, der an Speicherzelle \smalltt{Tmp(Num('addr2'))} steht und speichert diese auf den \colorbold{Stack}. Die Berechnung ist abhängig davon ob der \colorbold{Datentyp} \smalltt{ArrayDecl(datatype)} oder \smalltt{PntrDecl(datatype)} ist. Der \colorbold{Datentyp} ist ein \colorbold{verstecktes Attribut} von \smalltt{Ref(exp)}. \\
  \smalltt{Ref(Attr(Tmp(Num('addr1')), Name('attr')))} & Berechnet die nächste \colorbold{Adresse} aus der \colorbold{Adresse}, die an Speicherzelle \smalltt{Tmp(Num('addr1'))} steht und dem \colorbold{Attributnamen} \smalltt{Name('attr')} und speichert diese auf den \colorbold{Stack}. Zur Berechnung ist der Name des \colorbold{Struct} in \smalltt{StructSpec(Name('st'))} notwendig, dessen \colorbold{Attribut} \smalltt{Name('attr')} ist. \smalltt{StructSpec(Name('st'))} ist ein \colorbold{verstecktes Attribut} von  \smalltt{Ref(exp)}. \\
  \smalltt{Assign(Tmp(Num('size'))), GlobalRead(Num('addr')))} & Schreibt \smalltt{Num('size')} viele Speicherzellen, die ab \smalltt{GlobalRead(Num('addr'))} relativ zum \colorbold{Datensegment Register}  \smalltt{DS} stehen, versetzt genauso auf den \colorbold{Stack}. \\
  \smalltt{Assign(Tmp(Num('size')), StackRead(Num('addr')))} & Schreibt \smalltt{Num('size')} viele Speicherzellen, die ab \smalltt{GlobalRead(Num('addr'))} relativ zum \colorbold{Begin-Aktive-Funktion Register} \smalltt{BAF} stehen, versetzt genauso auf den \colorbold{Stack}. \\
  \smalltt{Exp(GlobalRead(Num('addr'))} & Speichert \colorbold{Inhalt} der Speicherzelle, die \smalltt{Num('addr')} Speicherzellen \colorbold{relativ} zum \colorbold{Datensegment Register} \smalltt{DS} steht auf den \colorbold{Stack}. \\
  \smalltt{Exp(StackRead(Num('addr'))} & Speichert \colorbold{Inhalt} der Speicherzelle, die \smalltt{Num('addr')} Speicherzellen \colorbold{relativ} zum \colorbold{Begin-Aktive-Funktion Register} \smalltt{BAF} steht auf den \colorbold{Stack}. \\
  \smalltt{Exp(Tmp(Num('addr')))} & Speichert \colorbold{Inhalt} der Speicherzelle, die \smalltt{Num('addr')} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer Register}  \smalltt{SP} steht auf den \colorbold{Stack}. \\
  \smalltt{Assign(Tmp(Num('addr1')), Tmp(Num('addr2')))} & Speichert \colorbold{Inhalt} der Speicherzelle \smalltt{Tmp(Num('addr2'))}, die \smalltt{Num('addr2')} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer Register}  \smalltt{SP} steht an der \colorbold{Adresse} in der Speicherzelle, die \smalltt{Num('addr1')} Speicherzellen \colorbold{relativ} zum \colorbold{Stackpointer Register}  \verb|SP| steht. \\
  \smalltt{Assign(GlobalWrite(Num('addr')), Tmp(Num('size')))} & Schreibt \smalltt{Num('size')} viele Speicherzellen, die auf dem \colorbold{Stack} stehen, versetzt genauso auf die Speicherzellen ab \smalltt{Num('addr')} \colorbold{relativ} zum \colorbold{Datensegment Register}  \smalltt{DS}. \\
  \smalltt{Assign(StackWrite(Num('addr')), Tmp(Num('size')))} & Schreibt \smalltt{Num('size')} viele Speicherzellen, die auf dem \colorbold{Stack} stehen, versetzt genauso auf die Speicherzellen ab \smalltt{Num('addr')} \colorbold{relativ} zum \colorbold{Begin-Aktive-Funktion Register} \smalltt{BAF}. \\
...
\begin{Special_Paragraph}
  Um die obige Tabelle~\ref{tab:kompositionen_von_picoc_knoten_und_reti_knoten_mit_besonderer_bedeutung} nicht mit unnötig viel repetetiven Inhalt zu füllen, wurden die zahlreichen Kompostionen \colorbold{ausgelassen}, bei denen einfach nur \verb|exp| durch $\mathtt{Tmp(Num('x')), x}\in\mathbb{N}$ ersetzt wurde.
