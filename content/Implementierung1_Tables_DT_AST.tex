...
\begin{Special_Paragraph}
  Die \colorbold{Abstrakte Syntax} der Sprache $L_{Picoc\_Blocks}$ in Tabelle~\ref{gr:abstract_syntax_l_picoc_blocks} ist als \colorbold{Patch}\footnote{Das ist ein \colorbold{Deutsches Wort}, wie Quelle~\cite{noauthor_duden_nodate} zeigt. Es gibt \colorbold{kein} anderes Wort, welches \colorbold{genau} das ausdrückt, was dieses Wort ausdrückt.} anzusehen. Ein \colorbold{Patch} ist es insofern, dass
  \begin{itemize}
    \item es neue Knoten \colorbold{hinzufügt}, die zur Umsetzung dieses Passes notwendig sind, wie z.B. den \smalltt{GoTo(Name(str))}-Knoten
    \item es bestehende Knoten \colorbold{abändert}, weil , wie z.B. den \smalltt{FunDef(⟨datatype⟩, Name(str), Alloc(Writeable(), ⟨datatype⟩, Name(str))*, ⟨block⟩*)}-Knoten
  \end{itemize}
