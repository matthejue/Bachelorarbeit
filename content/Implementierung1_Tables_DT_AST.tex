...
Daher wandelt der \colorbold{PicoC-Shrink Pass} alle Verwendungen des \colorbold{Knoten} \smalltt{Deref(exp, i)} im jeweiligen \colorbold{Abstract Syntax Tree} in \colorbold{Knoten} \smalltt{Subscr(exp, i)} um, sodass sich dadurch viele vermeidbare \colorbold{Fallunterscheidungen} und \colorbold{doppelter Code} bei der Implementierung vermeiden lassen, denn man kann die \colorbold{Derefenzierung} \smalltt{*(var + i)} einfach von den Routinen für einen \colorbold{Zugriff auf einen Arrayindex} \smalltt{var[i]} übernehmen lassen.
...
Die \colorbold{Abstrakte Syntax} der Sprache $L_{PicoC\_Shrink}$ ist \colorbold{identisch} mit der \colorbold{Abstrakten Syntax} der Sprache $L_{PiocC}$ aus Tabelle~\ref{gr:abstract_syntax_l_picoc}, nach welcher der \colorbold{erste} Abstract Syntax Tree in der \colorbold{Syntaktischen Analyse} generiert wurde. Das liegt daran, dass dieser Pass nur alle \colorbold{Vorkommnisse} eines Knoten \smalltt{Deref(exp, i)} durch den Knoten \smalltt{Subscr(exp, i)} auswechselt, der ebenfalls bereits in der \colorbold{Abstrakten Syntax} der Sprache $L_{PicoC}$ definiert ist.
...
\begin{Special_Paragraph}
  ...
  % Die \colorbold{Abstrakte Syntax} der Sprache $L_{Picoc\_Blocks}$ in Tabelle~\ref{gr:abstract_syntax_l_picoc_blocks} ist als \colorbold{Erweiterung} anzusehen. Ein \colorbold{Erweiterung} ist es insofern, dass es neue Knoten \colorbold{hinzufügt}, die zur Umsetzung dieses Passes notwendig sind, wie z.B. den \smalltt{GoTo(Name(str))}-Knoten. Den Knoten \smalltt{FunDef(⟨datatype⟩, Name(str), Alloc(Writeable(), ⟨datatype⟩, Name(str))*, ⟨block⟩*)} könnte man als Änderung des bestehenden Knoten \smalltt{FunDef(⟨datatype⟩, Name(str), Alloc(Writeable(), ⟨datatype⟩, Name(str))*, ⟨stmt⟩*)} ansehen. Aber da die Abstrakte Syntax vor allem für den Programmierer relevant ist, anders als die Konkrette Syntax, die von einem Parser Generator ausgewertet wird und daher perfekt definiert sein muss

% \smalltt{If(⟨exp⟩, ⟨stmt⟩*)}, \smalltt{IfElse(⟨exp⟩, ⟨stmt⟩*, ⟨stmt⟩*)}, \smalltt{While(⟨exp⟩, ⟨stmt⟩*)} und \smalltt{DoWhile(⟨exp⟩, ⟨stmt⟩*)}

  % vor allem für Programmierer muss nicht syntaktisch genau sein, wenn es Programmierer damit versteht, was er tuen soll, dann ist es gut so, deswegen nicht nötig anzugeben, was für Knoten nicht mehr existieren.

  Man bezeichnet hier die Abstrakte Syntax als \enquote{Abstrakte Syntax der Sprache $L_{Picoc\_Blocks}$}. Diese Sprache $L_{Picoc\_Blocks}$ besitzt eine \colorbold{Konkrette Syntax} und
\end{Special_Paragraph}
...
\begin{Special_Paragraph}
  % die Sache mit Konkretter Syntax statt Abstrakter Syntax da so simpel und so....
\end{Special_Paragraph}
...
\begin{grammar}[Abstrakte Syntax für $L_{RETI\_Patch}$][H][gr:abstract_syntax_l_reti_blocks]
  \toprule
  \retiblocks*
  \midrule
  \picocblocksleftover*
