...
\begin{Special_Paragraph}
  Man spricht hier von der \colorbold{\dq Abstrakten Syntax der Sprache $L_{PicoC}$\dq}, aber man hat bei der \colorbold{Konkretten Syntax}~\ref{gr:concrete_syntax_lex_teil_1} auch schon von der \colorbold{\dq Konkretten Syntax der Sprache $L_{PicoC}$\dq} gesprochen. Hierbei handelt es sich um eine im Compilerbau geläufige \colorbold{Redeart}, die vielleicht verwirrend sein kann, da die \colorbold{Konkrette Syntax} und die \colorbold{Abstrakte Syntax} zwei \colorbold{verschiedene Grammatiken} sind, die für sich genommen zwei \colorbold{unterschiedliche} Sprachen beschreiben. Die Sprache $L_{PicoC}$ wird von ihrer \colorbold{Konkretten Syntax} komplett beschrieben. Allerdings will man mit der \colorbold{ersteren Redeart} ausdrücken, dass die \colorbold{Abstrakte Syntax} die zur Sprache $L_{PicoC}$ passend definierte Grammatik ist, die beschreibt, was für \colorbold{Kompositionen} mit den \colorbold{PicoC-Knoten} möglich sind, die für die Kompilierung dieser Sprache $L_{PicoC}$ definiert wurden. Für die tatsächliche Sprache, die durch die \colorbold{Abstrakten Syntax} beschrieben wird, interessiert man sich nicht explizit. Diese \colorbold{Redeart} wurde aus der \colorbold{Quelle} \cite{g_siek_course_2022} übernommen.
\end{Special_Paragraph}
