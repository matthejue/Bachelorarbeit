...
Nachdem der \colorbold{Derviation Tree} in Unterkapitel~\ref{sec:derivation_tree_vereinfachung} vereinfacht wurde, ist der \colorbold{vereinfachte Derivation Tree} in Code~\ref{code:picoc_code_nach_derivation_tree_vereinfachung} nun dazu geeignet, um mit einem \colorbold{Transformer} einen \colorbold{Abstract Syntax Tree} aus ihm zu generieren.

Hierzu kommt ein \colorbold{Transformer} (Definition~\ref{def:transformer}) zu Einsatz, der aus dem \colorbold{Derivation Tree} in Code~\ref{} einen \colorbold{Abstract Syntax Tree} in Code~\ref{fig:abstract_syntax_tree_generierung_ohne_umdrehen} generiert.

\smalltt{struct st *(*var[3][2])}
