...
Zur Umsetzung dieses Passes ist es notwendig die \colorbold{Abstrakte Syntax} der Sprache $L_{PicoC\_Blocks}$ in Tabelle~\ref{gr:abstract_syntax_l_picoc_blocks} in die \colorbold{A-Normalform} zu bringen. Darunter fällt es, dafür zu sorgen, dass \colorbold{Komplexe Knoten}, wie z.B. \smalltt{BinOp(exp, bin\_op, exp)} nur \colorbold{Atomare Knoten}, wie z.B. \smalltt{Stack(Num(str))} haben können. Des Weiteren werden auch \colorbold{Funktionen} und \colorbold{Funktionsaufrufen} aufgelöst, sodass u.a. die Blöcke \smalltt{Block(Name(str), stmt*)} nun direkt im \smalltt{File(Name(str), block*)}-Knoten liegen usw., was in Unterkapitel~\ref{sec:umsetzung_von_funktionen} genauer erklärt wird.

Zudem wird eine \colorbold{Symboltabelle} (Definition~\ref{def:symboltabelle}) eingeführt, welche dazu da Informationen, wie Adresse und Datentyp von Variablen zu speichern.

Das resultiert in der \colorbold{Abstrakten Syntax} der Sprache $L_{PicoC\_ANF}$ in Tabelle~\ref{gr:abstract_syntax_l_picoc_anf}.
...
Eine über ein \colorbold{Assoziatives Feld} umgesetzte \colorbold{Datenstruktur}, die jedem \colorbold{Bezeichner} in der \colorbold{Symboltabelle}


zu \colorbold{Variablen}, \colorbold{Konstanten} und \colorbold{Funktionen}, deren \colorbold{Bezeichner} in der Symboltabelle als \colorbold{Symbole} bezeichnet werden, Informationen, wie die \colorbold{Adresse} oder den \colorbold{Datentyp} abspeichert.



die in einem Programm deklariert oder definiert sind, Informationen, wie die Adresse oder den Datentyp

  Die Symboltabelle muss nur während des \colorbold{Kompiliervorgangs} im \colorbold{Speicher} existieren
\end{Definition}

\newlineparagraph{Codebeispiel}

In Code~\ref{code:picoc_mon_pass_für_codebeispiel} sieht man den \colorbold{Abstract-Syntax-Tree} des \colorbold{PiocC-ANF Passes} für das aus Unterkapitel~\ref{code:picoc_code_für_codebeispiel} weitergeführte Beispiel, indem
