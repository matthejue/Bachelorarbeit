...
Der Knoten \smalltt{Ref(Name('var')))} repräsentiert im \colorbold{Abstrakt Syntax Tree} in Code~\ref{code:abstract_syntax_tree_für_pointer_referenzierung} eine \colorbold{Referenzierung} (z.B. \verb|&var|).
...
Der Knoten \smalltt{Deref(Name('var')))} repräsentiert im \colorbold{Abstrakt Syntax Tree} in Code~\ref{code:abstract_syntax_tree_für_pointer_dereferenzierung} eine \colorbold{Dereferenzierung} (z.B. \smalltt{*var}).
...
Der \colorbold{Zugriff auf einen Arrayindex} (z.B. \smalltt{ar[0]}) wird im  \colorbold{Abstract Syntx Tree} in Code~\ref{code:abstract_syntax_tree_für_zugriff_auf_arrayindex} mithilfe des \colorbold{Container-Knotens} \smalltt{Subscr(Name('ar'), Num('0'))} dargestellt.
...
Im \colorbold{Abstract Syntax Tree} in Code~\ref{code:abstract_syntax_tree_für_array_assignment} wird eine \colorbold{Zuweisung} an einen \colorbold{Arrayindex} (z.B. \smalltt{ar[2] = 42;}) durch die Komposition \smalltt{Assign(Subscr(Name('ar'), Num('2')), Num('42'))} dargestellt.
...
\subsubsection{Deklaration von Structtypen}

Die \colorbold{Deklaration} eines neues Structtyps\smalltt{1 struct st1 {int *ar[3];};} wird im Folgenden anhand des Beispiels in Code~\ref{code:picoc_code_für_deklaration_von_structs} erläutert.
