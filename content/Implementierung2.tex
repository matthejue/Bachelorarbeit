...
Im Folgenden ist der Ablauf identisch zur \colorbold{Initialisierung eines Arrays} in Subkapitel~\ref{sec:initialisierung_von_arrays}, daher wird um keine Reptition zu betreiben auf Subkapitel~\ref{sec:initialisierung_von_arrays} verwiesen. Der einzige Unterschied ist, dass der im \colorbold{Abstract Syntax Tree} am weitesten oben liegende \colorbold{Intializer}-Knoten ein \colorbold{Struct Initializer} sein muss und kein \colorbold{Array Initializer}. Dafür wird in diesem Beispiel in Code~\ref{code:abstract_syntax_tree_für_initialisierung_von_structs} eine komplexere, mehrstufige Initialisierung mit \colorbold{verschiedenen} Datentypen erklärt.

Im \colorbold{PicoC-Mon Pass} in Code~\ref{code:picoc_mon_pass_für_initialisierung_von_structs} werden die \colorbold{Logischen Ausdrücke} in den Blättern des vom \colorbold{Struct-Initializer} \colorbold{Container-Knoten} ausgehenden Baumes \smalltt{Struct([Assign(Name('attr1'), Name('var')), Assign(Name('attr2'), Struct([Assign(Name('attr'), Array([Array([Ref(Name('var')), Ref(Name('var'))])]))]))])} genauso, wie der \colorbold{Array Initializer}, wie in Subkapitel~\ref{sec:initialisierung_von_arrays} nach dem \colorbold{Depth-First-Search} Schema, von \colorbold{links-nach-rechts} ausgewertet. Im \colorbold{Struct-Initializer} sind weitere \colorbold{Struct- oder Array-Initializer} und \colorbold{Logische Ausdrücke} immer im rechten \colorbold{exp} Eintrag des Arrays Container-Kontens zu finden: \smalltt{Assign(Name('attr1'), Name('var'))}.
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={18-20,22-25,27-30,32-36}}]{./code_examples/example_struct_init.reti_blocks}
