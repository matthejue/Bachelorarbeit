...
\begin{Special_Paragraph}
  Sei $\mathtt{datatype_{i, idx_i}}$ der \colorbold{Datentyp} eines \colorbold{Elementes} / \colorbold{Attributes} einer \colorbold{komplexen Stuktur} im Hauptspeicher, deren Elemente / Attribute sich in eine \colorbold{Baumstruktur} einordnen lassen\footnote{Damit ist der \colorbold{Abstract Syntax Tree} des \colorbold{Datentyps} der \colorbold{Variable} gemeint.}, die von \colorbold{oben-nach-unten} wächst. Dabei steht \smalltt{i} für eine \colorbold{Ebene} der Baumstuktur, wobei \smalltt{i=1} die \colorbold{oberste} Ebene ist und $\mathtt{idx_i}$ die \colorbold{Nummer} eines beliebigen \colorbold{Elementes / Attributes} des  ist.\footnote{Als Beispiel ist $\mathtt{datatype_{2, 3}}$ folglich der \colorbold{Datentyp} des $3$ten Elements / Attributes in der Ebene $2$}

  Die Berechnung der \colorbold{Adresse}, für eine beliebige Aneinanderreihung verschiedener \colorbold{Zugriffe} auf \colorbold{Pointerelemente}, \colorbold{Arrayelemente} und \colorbold{Structattributte} $\mathtt{complex\_var\enspace zug_{1, idx_1}\enspace\ldots\enspace zug_{n, idx_n}}$ unterschiedlicher Datentypen $\mathtt{datatype_{i, idx_i}}$, kann mittels der Formel~\ref{eq:adresse_von_derived_datatype_allgemein}:
  % https://tex.stackexchange.com/questions/21290/how-to-make-left-right-pairs-of-delimiter-work-over-multiple-lines
  \begin{dmath}
  \mathtt{ref(complex\_var\enspace zug_{1, idx_1}\enspace\ldots\enspace zug_{n, idx_n})} = \mathtt{ref(complex\_var)} + \sum_{i=1}^{n}\sum_{k=1}^{idx_i - 1} \mathtt{\operatorname{size}(datatype_{i, k})}
