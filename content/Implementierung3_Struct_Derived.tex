...
\begin{Special_Paragraph}
  Sei $\mathtt{datatype_{i}}$ ein \colorbold{Knoten} eines \colorbold{entarteten Baumes} (siehe Definition~\ref{def:entarteter_baum} und Abbildung~\ref{eq:datatype_tree}), dessen Wurzel $\mathtt{datatype_{1}}$ ist. Dabei steht \smalltt{i} für eine \colorbold{Ebene} des entarteten Baumes. Die Knoten des entarteten Baumes lassen sich \colorbold{Startadressen} $\mathtt{ref(datatype_{i})}$ von Speicherbereichen $\mathtt{ref(datatype_{i})\enspace\ldots\enspace ref(datatype_{i}) + size(datatype_{i})}$ im \colorbold{Hauptspeicher} zuordnen, wobei gilt, dass $\mathtt{ref(datatype_{i}) \leq ref(datatype_{i+1}) < ref(datatype_{i}) + size(datatype_{i})}$.\footnote{Es ist ein Baum, der \colorbold{nur} die \colorbold{Datentypen} als Knoten enthält, auf die \colorbold{zugegriffen} wird.}\footnote{\smalltt{ref(datatype)} steht dabei für das Schreiben der \colorbold{Startadresse}, die dem \colorbold{Datentyp} \smalltt{datatype} zugeordnet ist auf den \colorbold{Stack}.}

  Sei $\mathtt{datatype_{i, k}}$ ein beliebiges \colorbold{Element / Attribut} des \colorbold{Datentyps} $\mathtt{datatype_i}$. Dabei gilt: $\mathtt{ref(datatype_{i, k}) < ref(datatype_{i, k+1})}$.

  Sei $\mathtt{datatype_{i, idx_i}}$ ein beliebiges \colorbold{Element / Attribut} des \colorbold{Datentyps} $\mathtt{datatype_i}$, sodass gilt: $\mathtt{datatype_{i, idx_i} = datatype_{i+1}}$.
  ...
  Die Berechnung der \colorbold{Adresse} für eine beliebige Folge verschiedener Datentypen $(\mathtt{datatype_{1, idx_1},\enspace\ldots,\enspace datatype_{n, idx_n}})$, die das Resultat einer Aneinandereihung von \colorbold{Zugriffen} auf \colorbold{Pointerelemente}, \colorbold{Arrayelemente} und \colorbold{Structattributte} unterschiedlicher Datentypen $\mathtt{datatype_{i}}$ ist (z.B. \smalltt{*complex\_var.attr3[2]}), kann mittels der Formel~\ref{eq:adresse_von_derived_datatype_allgemein}:
  ...
  \end{equation}
  berechnet werden.\footnote{Die \colorbold{äußere Schleife} iteriert nacheinander über die Folge von Datentypen, die aus den \colorbold{Zugriffen} auf \colorbold{Pointerelmente}, \colorbold{Arrayelmente} oder \colorbold{Structattribute} resultiert. Die \colorbold{innere Schleife} iteriert über alle \colorbold{Elemente} oder \colorbold{Attribute} des momentan betrachteten \colorbold{Datentyps} $\mathtt{datatype_i}$, die vor dem \colorbold{Element} / \colorbold{Attribut} $\mathtt{datatype_{i, idx_i}}$ liegen.}

  Dabei darf nur der letzte Knoten $\mathtt{datatype_n}$ vom Datentyp \colorbold{Pointer} sein. Ist in einer Folge von  \colorbold{Datentypen} ein Knoten vom Datentyp \colorbold{Pointer}, der nicht der \colorbold{letzte Datentyp} $\mathtt{datatype_n}$ in der Folge ist, so muss die \colorbold{Adressberechnung} in $2$ Adressberechnungen aufgeteilt werden, wobei die \colorbold{erste Adressberechnung} vom ersten Datentyp $\mathtt{datatype_1}$ bis direkt zum Dantentyp Pointer geht $\mathtt{datatype_{pntr}}$ und die \colorbold{zweite Adressberechnung} einen Dantentyp nach dem Datentyp Pointer anfängt $\mathtt{datatpye_{pntr+1}}$ und bis zum letzten Datenyp $\mathtt{datatype_n}$ geht. Bei der \colorbold{zweiten Adressberechnung} muss dabei die \colorbold{Adresse} $\mathtt{ref(datatype_1)}$ des Summanden aus der Formel~\ref{eq:adresse_von_derived_datatype_allgemein} auf den Inhalt der Speicherzelle an der gerade in der \colorbold{zweiten Adressberechnung} berechneten Adresse $\mathtt{M\left[ref(datatype_1,\enspace\ldots,\enspace datatype_{pntr})\right]}$ gesetzt werden.

  Die Formel~\ref{eq:adresse_von_derived_datatype_allgemein} stellt dabei eine \colorbold{Verallgemeinerung} der Formel~\ref{eq:adresse_von_arrayelement} dar, die für alle möglichen Aneinandereihungen von Zugriffen auf \colorbold{Pointerelemente}, \colorbold{Arrayelementen} und \colorbold{Structattribute} funktioniert (z.B. \smalltt{(*complex\_var.attr2)[3]}). Da die Formel \colorbold{allgemein} sein muss, lässt sie sich nicht so elegant mit einem Produkt $\prod$ schreiben, wie die Formel~\ref{eq:adresse_von_arrayelement}, da man nicht davon ausgehen kann, dass alle Elemente den gleichen Datentyp haben\footnote{Structattribute haben \colorbold{unterschiedliche} Größen.}.

  Die Komposition \smalltt{Ref(Global(num))} bzw. \smalltt{Ref(Stackframe(num))} repräsentiert dabei den Summanden $\mathtt{ref(datatype_1)}$ in der Formel.

  Die Komposition \smalltt{Exp(Attr(Stack(Num('1')), name))} repräsentiert dabei einen Summanden $\sum_{k=1}^{idx_i - 1} \mathtt{\operatorname{size}(datatype_{i, k})}$ in der Formel.

Die Komposition \smalltt{Exp(Stack(Num('1')))} repräsentiert dabei das Lesen des \colorbold{Inhalts} $\mathtt{M\left[ref(datatype_{1, idx_1},\enspace\ldots,\enspace datatype_{n, idx_n})\right]}$ der Speicherzelle an der finalen \colorbold{Adresse} $\mathtt{ref(datatype_{1, idx_1},\enspace\ldots,\enspace datatype_{n, idx_n})}$.
\end{Special_Paragraph}
...
Im \colorbold{PicoC-ANF Pass} in Code~\ref{code:picoc_mon_pass_einleitungsteil} wird die Komposition \smalltt{Exp(Ref(Name('complex\_var')))} durch die Komposition \smalltt{Ref(Global(Num('9')))} bzw. \smalltt{Ref(Stackframe(Num('9')))} ersetzt, je nachdem, ob die Variable \smalltt{Name('complex\_var')} in den \colorbold{Globalen Statischen Daten} oder auf dem \colorbold{Stack} liegt.

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={8,15}}]{./code_examples/example_derived_dts_introduction_part.picoc_mon}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={9-12,21-24}}]{./code_examples/example_derived_dts_introduction_part.reti_blocks}
...
Das Generieren des \colorbold{Abstract Syntax Tree} in Code~\ref{code:abstract_syntax_tree_schlussteil} verläuft wie üblich.
...
Im \colorbold{PicoC-ANF Pass} in Code~\ref{code:picoc_mon_pass_schlussteil} wird das eben angesprochene auf den \colorbold{Stack} speichern des \colorbold{Inhalts} der berechneten \colorbold{Adresse} mit der Komposition \smalltt{Exp(Stack(Num('1')))} dargestellt.

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={20,25,30}}]{./code_examples/example_derived_dts_final_part.picoc_mon}
  \caption{PicoC-ANF Pass für den Schlussteil}
  \label{code:picoc_mon_pass_schlussteil}
\end{code}

Im \colorbold{RETI-Blocks Pass} in Code~\ref{code:reti_blocks_pass_schlussteil} wird die Komposition \smalltt{Exp(Stack(Num('1')))} durch ihre entsprechenden \colorbold{RETI-Knoten} ersetzt, wenn das \textcolor{gray!90!black}{versteckte Attribut} \smalltt{datatype} im \smalltt{Exp(exp,\textcolor{gray!90!black}{datatpye})}-Container-Knoten kein \colorbold{Array} \smalltt{ArrayDecl(nums, datatype)} enthält, ansonsten ist bei einem \colorbold{Array} die \colorbold{Adresse} auf dem \colorbold{Stack} bereits das gewünschte Ergebnis. Genaueres wurde in Unterkapitel~\ref{sec:übersicht} zusammen mit der Abbildung~\ref{fig:zugriff_auf_derived_datatypes_veranschaulicht} bereits erklärt.

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={53,72-74,94-96}}]{./code_examples/example_derived_dts_final_part.reti_blocks}
