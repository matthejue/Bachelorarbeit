...
\begin{Special_Paragraph}
  Sei $\mathtt{location_{i}}$ eine \colorbold{Location} (Definition~\ref{def:location}) und ein \colorbold{Knoten} eines \colorbold{entarteten Baumes} (siehe Definition~\ref{def:entarteter_baum} und Abbildung~\ref{eq:location_tree}), dessen Wurzel $\mathtt{location_{1}}$ ist. Dabei steht \smalltt{i} für eine \colorbold{Ebene} des entarteten Baumes. Die Knoten des entarteten Baumes lassen sich \colorbold{Startadressen} $\mathtt{ref(location_{i})}$ von Speicherbereichen $\mathtt{ref(location_{i})\enspace\ldots\enspace ref(location_{i}) + size(location_{i})}$ im \colorbold{Hauptspeicher} zuordnen, wobei gilt, dass $\mathtt{ref(location_{i}) \leq ref(location_{i+1}) < ref(location_{i}) + size(location_{i})}$.\footnote{Es ist ein Baum, der \colorbold{nur} die \colorbold{Datentypen} als Knoten enthält, auf die \colorbold{zugegriffen} wird.}\footnote{\smalltt{ref(location)} steht dabei für das Schreiben der \colorbold{Adresse} von \smalltt{location} auf den Stack.}

  Jeder \colorbold{Location} $\mathtt{location_{i}}$ ist dabei ein \colorbold{Dantentyp} $\mathtt{datatype_i}$ zugeordnet.

  Sei $\mathtt{location_{i, k}}$ eine \colorbold{Location} der ein beliebiges \colorbold{Element / Attribut} des \colorbold{Datentyps} $\mathtt{datatype_i}$ der \colorbold{Location} $\mathtt{location_i}$ zugeordnet ist. Dabei gilt: $\mathtt{ref(location_{i, k}) < ref(location_{i, k+1})}$.

  Sei $\mathtt{location_{i, idx_i}}$ die \colorbold{Location} eines beliebigen \colorbold{Elementes / Attributes} des \colorbold{Datentyps} $\mathtt{datatype_i}$ der \colorbold{Location} $\mathtt{location_i}$, sodass gilt: $\mathtt{location_{i, idx_i} = location_{i+1}}$.
  ...
  \begin{equation}
  \mathtt{ref(datatype_{1, idx_1},\enspace\ldots,\enspace datatype_{n, idx_n})} = \mathtt{ref(location_1)} + \sum_{i=1}^{n-1}\sum_{k=1}^{idx_i - 1} \mathtt{\operatorname{size}(datatype_{i, k})}
