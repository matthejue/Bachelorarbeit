...
Im \colorbold{PicoC-Mon Pass} in Code~\ref{code:picoc_mon_pass_für_funktionsaufruf_ohne_rückgabewert} wird die Komposition und Container-Knoten \smalltt{Exp(Call(Name('stack\_fun'), [Name('local\_var')]))} durch die Kompositionen \smalltt{StackMalloc(Num('2'))}, \smalltt{Ref(Global(Num('0')))}, \smalltt{NewStackframe(Name('stack\_fun'), GoTo(Name('addr@next\_instr')))}, \smalltt{Exp(GoTo(Name('stack\_fun.0')))} und \smalltt{RemoveStackframe()} ersetzt, welche in den Tabellen~\ref{tab:kompositionen_von_picoc_knoten_und_reti_knoten_mit_besonderer_bedeutung} und \ref{tab:picoc_knoten_teil_1} genauer erklärt sind.
...
Im \colorbold{RETI-Blocks Pass} in Code~\ref{code:reti_blocks_pass_für_funktionsaufruf_ohne_rückgabewert} werden die Kompositionen \smalltt{Exp(Call(Name('stack\_fun'), [Name('local\_var')]))} durch die Kompositionen \smalltt{StackMalloc(Num('2'))}, \smalltt{Ref(Global(Num('0')))}, \smalltt{NewStackframe(Name('stack\_fun'), GoTo(Name('addr@next\_instr')))}, \smalltt{Exp(GoTo(Name('stack\_fun.0')))} und \smalltt{RemoveStackframe()} dann durch ihre entsprechenden \colorbold{RETI-Knoten} ersetzt.

Die \colorbold{Kompostionen} \smalltt{LOADI ACC GoTo(Name('addr@next\_instr'))} und \smalltt{Exp(GoTo(Name('stack\_fun.0')))} entsprechen noch keine fertigen \colorbold{RETI-Instructions} und werden später in dem für sie vorgesehenen \colorbold{RETI-Pass} passend ergänzt bzw. ersetzt.
...
Ein \colorbold{Funktionsaufruf inklusive Zuweisung eines Rückgabewertes} (z.B. \smalltt{int var = stack\_fun()}) wird im Folgenden mithilfe des Beispiels in Code~\ref{code:picoc_code_für_funktionsaufruf_mit_rückgabewert} erklärt.
% todo: unsichtbare return statements

\begin{code}
  \centering
  \numberedcodebox[minted language=c, minted options={highlightlines={2, 6}}]{./code_examples/example_fun_call_with_return_value.picoc}
  \caption{PicoC-Code für Funktionsaufruf mit Rückgabewert}
  \label{code:picoc_code_für_funktionsaufruf_mit_rückgabewert}
\end{code}

Im \colorbold{Abstract Syntax Tree} in Code~\ref{code:abstract_syntax_tree_für_funktionsaufruf_mit_rückgabewert} wird ein \colorbold{Return-Statement} \smalltt{return 21 * 2} mit der \colorbold{Komposition} \smalltt{Return(BinOp(Num('21'), Mul('*'), Num('2')))} dargestellt und ein \colorbold{Funktionsaufruf} inklusive \colorbold{Zuweisung des Rückgabewertes} \smalltt{int var = stack\_fun()} durch die Komposition \smalltt{Assign(Alloc(Writeable(), IntType('int'), Name('var')), Call(Name('stack\_fun'), []))}.

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={9, 16}}]{./code_examples/example_fun_call_with_return_value.ast}
...
Im \colorbold{PicoC-Mon Pass} in Code~\ref{code:picoc_mon_pass_für_funktionsaufruf_mit_rückgabewert} wird bei der \colorbold{Komposition} \smalltt{Return(BinOp(Num('21'), Mul('*'), Num('2')))} erst die \colorbold{Expression} \smalltt{BinOp(Num('21'), Mul('*'), Num('2'))} zu \smalltt{Exp(BinOp(Stack(Num('2')), Mul('*'), Stack(Num('1'))))} ausgewertet.

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={8-11, 21-22}}]{./code_examples/example_fun_call_with_return_value.picoc_mon}
...
Im \colorbold{RETI-Blocks Pass} in Code~\ref{code:reti_blocks} werden

\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={9-11,13-15,17-21,23-25,49-51,53}}]{./code_examples/example_fun_call_with_return_value.reti_blocks}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=c, minted options={highlightlines={7}}]{./code_examples/example_fun_call_by_sharing_array.picoc}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={19,28,46,55}}]{./code_examples/example_fun_call_by_sharing_array.st}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={13-14}}]{./code_examples/example_fun_call_by_sharing_array.picoc_mon}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={16-19,21-24}}]{./code_examples/example_fun_call_by_sharing_array.reti_blocks}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={13}}]{./code_examples/example_fun_call_by_value_struct.picoc_mon}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={16-22}}]{./code_examples/example_fun_call_by_value_struct.reti_blocks}
