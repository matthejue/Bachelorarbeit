...
Die \colorbold{Komposition} \seqtt{Assign(Alloc(Writeable(), IntType('int'), Name('var')), Call(Name('fun\_with\_return\_value'), []))} wird nach dem allokieren der Variable \smalltt{Name('var')}, durch die Komposition \smalltt{Assign(Global(Num('0')), Stack(Num('1')))} ersetzt, welche den \colorbold{Rückgabewert} der Funktion \smalltt{Name('fun\_with\_return\_value')} aus dem  \smalltt{ACC}-Register rausholt und in der Variable \smalltt{Name('var')} speichert, indem er die \colorbold{Adresse} der Variable \smalltt{Name('var')} in der \colorbold{Symboltabelle} nachschlägt.
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={8-11, 16, 31-32}}]{./code_examples/example_fun_call_with_return_value.picoc_mon}
