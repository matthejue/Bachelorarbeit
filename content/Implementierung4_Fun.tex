...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={9-11,13-15,17-21,23-25,31,78}}]{./code_examples/example_fun_call_with_return_value.reti_blocks}
...
Im \colorbold{PicoC-Mon Pass} wird im Fall dessen, dass der \colorbold{oberste Container-Knoten} im Teilbaum, der den Datentyp darstellt und an die Funktion übergeben wird ein \colorbold{Array} \smalltt{ArrayDecl(nums, datatype)} ist, dieser zu einem \colorbold{Pointer} \smalltt{PntrDecl(num, datatype)} umgewandelt und der Rest des Teilbaumes, der am \smalltt{datatype}-Attribut hängt, an das \smalltt{datatype}-Attribut des \colorbold{Pointers} \smalltt{PntrDecl(num, datatype)} drangehängt. Diese \colorbold{Umwandlung} des \colorbold{Datentyps} kann in der \colorbold{Symboltabelle} in Code~\ref{code:symboltabelle_für_call_by_sharing_für_arrays} beobachtet werden. Die \colorbold{lokalen Variablen} \smalltt{local\_var1@main} und \smalltt{local\_var2@main} sind beide vom Datentyp \smalltt{ArrayDecl([Num('2'), Num('3')], IntType('int'))} und bei der Übergabe änder sich der Datentyp beider Variablen zu \smalltt{PntrDecl(Num('1'), ArrayDecl([Num('3')], IntType('int')))}.
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={15,19,33,37,51,55}}]{./code_examples/example_fun_call_by_sharing_array.st}
  \caption{Symboltabelle für Call by Sharing für Arrays}
  \label{code:symboltabelle_für_call_by_sharing_für_arrays}
\end{code}

Im \colorbold{PicoC-Mon Pass} in Code~\ref{code:picoc_mon_pass_für_call_by_sharing_für_arrays} ist zu sehen, dass zur Übergabe der beiden Arrays die \colorbold{Adresse} der Arrays auf den \colorbold{Stack} geschrieben wird. Die \colorbold{Adresse} der beiden Arrays auf den \colorbold{Stack} zu schreiben wird durch die Kompositionen \smalltt{Ref(Global(Num('0')))} und \smalltt{Ref(Global(Num('6')))} repräsentiert. Dabei stellen die Zahlen in den \colorbold{Container-Knoten} \smalltt{Global(num)} die \colorbold{relative Adressen} relativ zum \smalltt{DS}-Register dar, die aus der \colorbold{Symboltabelle} entnommen sind.
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={16-18,43-46}}]{./code_examples/example_fun_call_by_sharing_array.reti_blocks}
...
\begin{code}
  ...
  \numberedcodebox[minted language=text, minted options={highlightlines={15,19,33,37,51,55}}]{./code_examples/example_fun_call_by_sharing_array.st}
  \caption{Symboltabelle für Call by Sharing für Arrays}
  \label{code:symboltabelle_für_call_by_sharing_für_structs}
\end{code}
