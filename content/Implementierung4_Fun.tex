...
Ein \colorbold{Funktionsaufruf} (z.B. \smalltt{stack\_fun(local\_var)}), wird im Folgenden mithilfe des Beispiels in Code~\ref{code:picoc_code_für_funktionsaufruf_ohne_rückgabewert} erklärt. Das Beispiel ist so gewählt, dass alleinig der \colorbold{Funktionsaufruf} im Vordergrund steht und z.B. noch kein \colorbold{Rückgabewert} zurückgegeben wird\footnote{Was allerdings im nächsten Unterkapitel~\ref{sec:rückgabewert} erklärt wird.}.
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={10-14}}]{./code_examples/example_fun_call_no_return_value.picoc_mon}
...
\begin{code}
  \centering
  \numberedcodebox[minted language=text, minted options={highlightlines={11,13-16,18-25,27,29-31}}]{./code_examples/example_fun_call_no_return_value.reti_blocks}
